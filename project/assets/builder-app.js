...
(function () {
  ...
  function readingTimeStatic(p) {
    ...
    if (doCards) {
      sc += "var self=document.querySelector('.bxb-post-article');"
        + "var IN_POST='.bxb-post-article,.post-body,.entry-content';function isCard(a){if(a===self)return false;if(/bxb-post-article|bxb-post/.test(a.className||''))return false;if(a.closest&&a.closest(IN_POST))return false;return true;}"
      + "var cards=[].slice.call(document.querySelectorAll('article')).filter(isCard);"
  ...
  function bookmarkStatic() {
    ...
    var css = "<style>"
      // card button — rounded translucent pin, top-right of the thumbnail
      + ".bxb-bm{position:absolute;top:10px;right:10px;z-index:3;width:34px;height:34px;border-radius:10px;border:0;background:rgba(15,23,42,.46);backdrop-filter:blur(6px);display:grid;place-items:center;cursor:pointer;color:#fff;box-shadow:0 2px 10px rgba(0,0,0,.22);transition:transform .15s,background .15s;padding:0}"
      + ".bxb-bm:hover{transform:scale(1.08);background:rgba(15,23,42,.66)}"
      + ".bxb-bm svg{width:18px;height:18px;fill:none;stroke:currentColor;stroke-width:1.7}"
      + ".bxb-bm.on{background:var(--primary,#6366f1)}.bxb-bm.on svg{fill:#fff;stroke:#fff}"
      // inline button inside the post meta
      + ".bxb-bm-post{display:inline-flex;align-items:center;gap:5px;border:0;background:none;padding:0;cursor:pointer;color:var(--text-subtle,#64748b);font:inherit;font-size:13px}"
      + ".bxb-bm-post svg{width:15px;height:15px;fill:none;stroke:currentColor;stroke-width:1.8}"
      + ".bxb-bm-post.on{color:var(--primary,#6366f1)}.bxb-bm-post.on svg{fill:currentColor}"
      // header trigger button (top) with count badge
      + ".bxb-bm-top{position:relative;width:40px;height:40px;border-radius:10px;border:1px solid var(--border-med,#e8eaf2);background:transparent;color:var(--text-main,#1a1b1e);display:grid;place-items:center;cursor:pointer;flex:none;transition:background .15s,color .15s}"
      + ".bxb-bm-top:hover{background:var(--hover-bg,rgba(0,0,0,.05));color:var(--primary,#6366f1)}"
      + ".bxb-bm-top svg{width:20px;height:20px;fill:none;stroke:currentColor;stroke-width:1.7}"
      + ".bxb-bm-top.filled{color:var(--primary,#6366f1)}.bxb-bm-top.filled svg{fill:currentColor}"
      + ".bxb-bm-top .cnt{position:absolute;top:-5px;right:-5px;min-width:18px;height:18px;border-radius:9px;background:var(--primary,#6366f1);color:#fff;font-size:11px;font-weight:700;display:none;align-items:center;justify-content:center;padding:0 4px;line-height:1;box-shadow:0 0 0 2px var(--bg-header,#fff)}"
      + ".bxb-bm-top.bxb-bm-fixed{position:fixed;top:14px;right:14px;z-index:120;background:var(--bg-header,#fff);box-shadow:0 4px 16px rgba(0,0,0,.16)}"
      // saved-list panel — drops from the top (sheet on mobile, dropdown top-right on desktop)
      + ".bxb-bm-panel{position:fixed;inset:0;z-index:200;display:none}.bxb-bm-panel.open{display:block}"
      + ".bxb-bm-scrim{position:absolute;inset:0;background:rgba(0,0,0,.35)}"
      + ".bxb-bm-sheet{position:absolute;top:14px;left:14px;right:14px;max-height:78vh;background:var(--bg-header,#fff);color:var(--text-main,#1a1b1e);border-radius:16px;padding:16px;overflow:auto;box-shadow:0 18px 48px rgba(0,0,0,.26);border:1px solid var(--border,#e8eaf2);animation:bxbBmIn .2s ease}"
      + "@media(min-width:768px){.bxb-bm-sheet{left:auto;top:70px;right:20px;width:366px}}"
      + "@keyframes bxbBmIn{from{opacity:0;transform:translateY(-10px)}to{opacity:1;transform:none}}"
      + ".bxb-bm-head{display:flex;align-items:center;justify-content:space-between;margin:0 0 12px}"
      + ".bxb-bm-h{font-weight:700;margin:0;font-size:15px}"
      + ".bxb-bm-x{border:0;background:none;color:var(--text-subtle,#64748b);cursor:pointer;font-size:19px;line-height:1;padding:0}"
      + ".bxb-bm-item{display:flex;gap:10px;align-items:center;padding:9px 0;border-bottom:1px solid var(--border,#eef)}"
      + ".bxb-bm-item:last-child{border-bottom:0}"
      + ".bxb-bm-item img{width:56px;height:42px;object-fit:cover;border-radius:8px;flex:none;background:var(--bg-surface,#f1f2f9)}"
      ...
      + ".bxb-bm-empty{color:var(--text-subtle,#64748b);text-align:center;padding:28px 0;font-size:14px}"
      + "</style>";
    var sc = "<script>/*<![CDATA[*/(function(){"
      ...
      // top trigger → placed in the header bar (fallback: fixed top-right)
      + "var top=document.createElement('button');top.type='button';top.className='bxb-bm-top';top.setAttribute('aria-label','" + LSAVED + "');top.innerHTML=ICO+'<span class=\"cnt\"></span>';"
      + "var bar=document.querySelector('.site-bar');"
      + "if(bar){var bg=bar.querySelector('.nav-burger');if(bg){bar.insertBefore(top,bg);}else{bar.appendChild(top);}}else{top.classList.add('bxb-bm-fixed');document.body.appendChild(top);}"
      + "var panel=document.createElement('div');panel.className='bxb-bm-panel';panel.innerHTML='<div class=\"bxb-bm-scrim\"></div><div class=\"bxb-bm-sheet\"><div class=\"bxb-bm-head\"><div class=\"bxb-bm-h\">" + LSAVED + "</div><button type=\"button\" class=\"bxb-bm-x\" aria-label=\"close\">&#10005;</button></div><div class=\"bxb-bm-list\"></div></div>';document.body.appendChild(panel);"
      + "function openP(){paint();panel.classList.add('open');}function closeP(){panel.classList.remove('open');}"
      + "top.addEventListener('click',openP);"
      + "panel.querySelector('.bxb-bm-scrim').addEventListener('click',closeP);"
      + "panel.querySelector('.bxb-bm-x').addEventListener('click',closeP);"
      + "document.addEventListener('keydown',function(e){if(e.key==='Escape')closeP();});"
      + "function paint(){var a=get();var c=top.querySelector('.cnt');c.textContent=a.length;c.style.display=a.length?'flex':'none';top.classList.toggle('filled',a.length>0);"
      ...
      + "[].slice.call(document.querySelectorAll('.bxb-bm[data-u],.bxb-bm-post[data-u]')).forEach(function(b){b.classList.toggle('on',has(b.getAttribute('data-u')));});}"
      ...
      + "var btn=document.createElement('button');btn.type='button';btn.className='bxb-bm';btn.setAttribute('data-u',url);btn.setAttribute('aria-label','" + LSAVE + "');btn.innerHTML=ICO;"
      ...
      + "var pb=document.createElement('button');pb.type='button';pb.className='bxb-bm-post';pb.setAttribute('data-u',pu);pb.innerHTML=ICO+'<span>" + LSAVE + "</span>';"
  ...
  /* ---------- Image lightbox (post content images · click to zoom) ---------- */
  function lightboxStatic() {
    var css = "<style>"
      + ".bxb-lb{position:fixed;inset:0;z-index:9999;display:none;align-items:center;justify-content:center}"
      + ".bxb-lb.open{display:flex}"
      + ".bxb-lb-scrim{position:absolute;inset:0;background:rgba(0,0,0,.9);cursor:zoom-out}"
      + ".bxb-lb-img{position:relative;max-width:92vw;max-height:88vh;object-fit:contain;border-radius:6px;box-shadow:0 12px 44px rgba(0,0,0,.55);z-index:1}"
      + ".bxb-lb-close{position:absolute;top:16px;right:18px;z-index:2;width:42px;height:42px;border-radius:50%;border:0;background:rgba(255,255,255,.14);color:#fff;font-size:19px;cursor:pointer;line-height:1}"
      + ".bxb-lb-nav{position:absolute;top:50%;transform:translateY(-50%);z-index:2;width:46px;height:46px;border-radius:50%;border:0;background:rgba(255,255,255,.14);color:#fff;font-size:26px;cursor:pointer;line-height:1}"
      + ".bxb-lb-prev{left:14px}.bxb-lb-next{right:14px}"
      + ".bxb-lb-close:hover,.bxb-lb-nav:hover{background:rgba(255,255,255,.28)}"
      + ".bxb-lb-cap{position:absolute;bottom:18px;left:50%;transform:translateX(-50%);z-index:2;color:#fff;font-size:13px;background:rgba(0,0,0,.5);padding:6px 14px;border-radius:20px;max-width:82vw;text-align:center}"
      + ".bxb-post-article .post-body img{cursor:zoom-in}"
    ...
    var sc = "<script>/*<![CDATA[*/(function(){"
      ...
      + "if(/emoji|avatar|author-thumb|bxb-bm/.test(im.className||''))return false;"
      // the affiliate gallery brings its own viewer, so leave its pictures alone or both open at once
      ...
      + "var pic=ov.querySelector('.bxb-lb-img'),cap=ov.querySelector('.bxb-lb-cap'),cur=0;"
      ...
      + "ov.querySelector('.bxb-lb-next').addEventListener('click',function(e){e.stopPropagation();show(cur+1);});"
      + "if(imgs.length<2){ov.querySelector('.bxb-lb-prev').style.display='none';ov.querySelector('.bxb-lb-next').style.display='none';}"
      + "document.addEventListener('keydown',function(e){if(!ov.classList.contains('open'))return;if(e.key==='Escape')close();else if(e.key==='ArrowLeft')show(cur-1);else if(e.key==='ArrowRight')show(cur+1);});"
      + "}());/*]]>*/<\/script>";
    return css + sc;
  }

  /* ---------- Copy code button (adds a Copy button to every <pre> in a post) ---------- */
  function copycodeStatic() {
    var LBL = tpl("คัดลอก", "Copy"), DONE = tpl("คัดลอกแล้ว", "Copied");
    var css = "<style>"
      + ".bxb-post-article .post-body pre{position:relative}"
      + ".bxb-cc-btn{position:absolute;top:8px;right:8px;z-index:2;display:inline-flex;align-items:center;gap:5px;border:1px solid var(--border-med,rgba(128,128,128,.35));background:var(--bg-surface,rgba(255,255,255,.9));color:var(--text-main,#1a1b1e);font-family:inherit;font-size:12px;font-weight:500;padding:5px 10px;border-radius:8px;cursor:pointer;opacity:.9;transition:opacity .15s,color .15s;-webkit-tap-highlight-color:transparent}"
      + ".bxb-cc-btn:hover{opacity:1}"
      + ".bxb-cc-btn.ok{color:var(--primary,#16a34a)}"
      + ".bxb-cc-btn svg{width:14px;height:14px;flex:none;fill:none;stroke:currentColor;stroke-width:2}"
      + "</style>";
    var sc = "<script>/*<![CDATA[*/(function(){"
      + "var scope=document.querySelector('.bxb-post-article .post-body')||document.querySelector('.post-body');"
      + "if(!scope)return;"
      + "var CIC='<svg viewBox=\"0 0 24 24\" stroke-linecap=\"round\" stroke-linejoin=\"round\"><rect x=\"9\" y=\"9\" width=\"11\" height=\"11\" rx=\"2\"/><path d=\"M5 15V5a2 2 0 0 1 2-2h8\"/></svg>';"
      + "[].slice.call(scope.querySelectorAll('pre')).forEach(function(pre){"
      + "if(pre.querySelector('.bxb-cc-btn'))return;"
      + "if(getComputedStyle(pre).position==='static')pre.style.position='relative';"
      + "var btn=document.createElement('button');btn.type='button';btn.className='bxb-cc-btn';btn.innerHTML=CIC+'<span>" + LBL + "</span>';"
      + "btn.addEventListener('click',function(){var code=pre.querySelector('code')||pre;var txt=code.innerText||code.textContent||'';"
      + "function ok(){var sp=btn.querySelector('span');if(sp){var o=sp.textContent;sp.textContent='" + DONE + "';btn.classList.add('ok');setTimeout(function(){sp.textContent=o;btn.classList.remove('ok');},1400);}}"
      + "if(navigator.clipboard&&navigator.clipboard.writeText){navigator.clipboard.writeText(txt).then(ok,ok);}else{var ta=document.createElement('textarea');ta.value=txt;document.body.appendChild(ta);ta.select();try{document.execCommand('copy');}catch(e){}document.body.removeChild(ta);ok();}});"
      + "pre.appendChild(btn);});"
      + "}());/*]]>*/<\/script>";
    return css + sc;
  }
  ...
  function reviewkitCSS() {
    ...
    var xic = calloutIconUrl('<path d="M6 6l12 12M18 6 6 18"/>', "#dc2626");
    return ".bk-pc{display:grid;grid-template-columns:1fr 1fr;gap:16px;margin:1.5rem 0}"
      + "@media(max-width:560px){.bk-pc{grid-template-columns:1fr}}"
      + ".bk-pros,.bk-cons{border-radius:12px;padding:15px 18px;border:1px solid;font-style:normal;color:var(--text-main,#1a1b1e)}"
      + ".bk-pros{background:" + hexRgba("#16a34a", ".08") + ";border-color:" + hexRgba("#16a34a", ".28") + "}"
      + ".bk-cons{background:" + hexRgba("#dc2626", ".08") + ";border-color:" + hexRgba("#dc2626", ".28") + "}"
      + ".bk-pros>b:first-child,.bk-cons>b:first-child{display:block;font-size:15px;margin:0 0 10px}"
      + ".bk-pros>b:first-child{color:#16a34a}.bk-cons>b:first-child{color:#dc2626}"
      + ".bk-pros ul,.bk-cons ul,.bk-pros ol,.bk-cons ol{margin:0;padding:0;list-style:none}"
      + ".bk-pros li,.bk-cons li{position:relative;padding-left:27px;margin:7px 0;line-height:1.55}"
      + ".bk-pros li::before,.bk-cons li::before{content:'';position:absolute;left:0;top:.12em;width:19px;height:19px;background-repeat:no-repeat;background-position:center;background-size:contain}"
      + ".bk-pros li::before{background-image:" + chk + "}.bk-cons li::before{background-image:" + xic + "}"
      + ".bk-verdict{display:flex;gap:18px;align-items:center;border:1px solid var(--border,#e8eaf2);border-left:5px solid var(--primary,#6366f1);border-radius:12px;padding:16px 20px;margin:1.5rem 0;background:var(--bg-surface,#f7f8fc)}"
      + ".bk-verdict-score{flex:none;font-size:2.6rem;font-weight:800;line-height:1;color:var(--primary,#6366f1);min-width:2.2ch;text-align:center}"
      + ".bk-verdict-body{min-width:0}.bk-verdict-body b{font-size:16px;display:block;margin-bottom:2px;color:var(--text-main,#1a1b1e)}"
      + ".bk-stars{color:#f59e0b;letter-spacing:2px;font-size:15px;line-height:1}"
      + ".bk-verdict-body p{margin:6px 0 0;font-size:13.5px;color:var(--text-muted,#64748b);line-height:1.55}";
  }
  function reviewkitStatic() { return "<style>" + reviewkitCSS() + "</style>"; }

  /* ---------- FAQ accordion + FAQPage JSON-LD (author writes .bk-faq in the post) ---------- */
  function faqCSS() {
    return ".bk-faq{margin:1.6rem 0;display:flex;flex-direction:column;gap:10px}"
      + ".bk-faq details{border:1px solid var(--border,#e8eaf2);border-radius:12px;background:var(--bg-surface,#fff);overflow:hidden;transition:border-color .2s,box-shadow .2s}"
      + ".bk-faq details[open]{border-color:var(--primary,#6366f1);box-shadow:0 4px 18px " + hexRgba("#6366f1", ".08") + "}"
      + ".bk-faq summary{list-style:none;cursor:pointer;padding:15px 48px 15px 18px;position:relative;font-weight:600;color:var(--text-main,#1a1b1e);font-size:15.5px;line-height:1.5;-webkit-tap-highlight-color:transparent}"
      + ".bk-faq summary::-webkit-details-marker{display:none}"
      + ".bk-faq summary::after{content:'';position:absolute;right:19px;top:50%;width:10px;height:10px;margin-top:-7px;border-right:2px solid var(--text-subtle,#64748b);border-bottom:2px solid var(--text-subtle,#64748b);transform:rotate(45deg);transition:transform .25s,border-color .2s}"
      + ".bk-faq summary:hover,.bk-faq details[open] summary{color:var(--primary,#6366f1)}"
      + ".bk-faq details[open] summary::after{transform:rotate(-135deg);border-color:var(--primary,#6366f1)}"
      + ".bk-faq details>:not(summary){padding:0 18px 16px;margin:0;color:var(--text-muted,#4a5063);line-height:1.72;font-size:14.5px}"
      + ".bk-faq details>:not(summary):last-child{padding-bottom:17px}";
  }
  /* ---------- Image slider / gallery with captions (author writes .bk-slider) ---------- */
  function sliderCSS() {
    return ".bk-slider{position:relative;margin:1.7rem 0;border-radius:14px;overflow:hidden;background:#0b0f1a}"
      + ".bk-slider-track{display:flex;overflow-x:auto;scroll-snap-type:x mandatory;scrollbar-width:none;-ms-overflow-style:none;-webkit-overflow-scrolling:touch}"
      + ".bk-slider-track::-webkit-scrollbar{display:none}"
      + ".bk-slider figure{margin:0;flex:0 0 100%;scroll-snap-align:center;position:relative;min-width:0}"
      + ".bk-slider img{display:block;width:100%;aspect-ratio:16/10;object-fit:cover}"
      + ".bk-slider figcaption{position:absolute;left:0;right:0;bottom:0;background:linear-gradient(transparent,rgba(0,0,0,.78));color:#fff;padding:30px 16px 40px;font-size:13.5px;line-height:1.5}"
      + ".bk-slider-btn{position:absolute;top:calc(50% - 8px);transform:translateY(-50%);z-index:2;width:40px;height:40px;border-radius:50%;border:0;background:rgba(0,0,0,.42);color:#fff;cursor:pointer;display:grid;place-items:center;font-size:22px;line-height:1;transition:background .15s;-webkit-tap-highlight-color:transparent}"
      + ".bk-slider-btn:hover{background:rgba(0,0,0,.66)}"
      + ".bk-slider-prev{left:10px}.bk-slider-next{right:10px}"
      + ".bk-slider-dots{position:absolute;bottom:12px;left:50%;transform:translateX(-50%);z-index:2;display:flex;gap:6px}"
      + ".bk-slider-dots button{width:8px;height:8px;border-radius:50%;border:0;background:rgba(255,255,255,.5);cursor:pointer;padding:0;transition:width .2s,background .2s}"
      + ".bk-slider-dots button.on{background:#fff;width:20px;border-radius:4px}";
  }
  function sliderStatic() {
    var css = "<style>" + sliderCSS() + "</style>";
    var sc = "<script>/*<![CDATA[*/(function(){"
      + "var list=[].slice.call(document.querySelectorAll('.bk-slider'));"
      + "list.forEach(function(sl){"
      + "if(sl.getAttribute('data-bxb'))return;sl.setAttribute('data-bxb','1');"
      + "var figs=[].slice.call(sl.querySelectorAll(':scope > figure'));if(!figs.length)return;"
      + "var track=document.createElement('div');track.className='bk-slider-track';"
      + "figs.forEach(function(f){track.appendChild(f);});sl.appendChild(track);"
      + "if(figs.length<2)return;"
      + "var cur=0;"
      + "var prev=document.createElement('button');prev.type='button';prev.className='bk-slider-btn bk-slider-prev';prev.setAttribute('aria-label','" + tpl("ก่อนหน้า", "Previous") + "');prev.innerHTML='\\u2039';"
      + "var next=document.createElement('button');next.type='button';next.className='bk-slider-btn bk-slider-next';next.setAttribute('aria-label','" + tpl("ถัดไป", "Next") + "');next.innerHTML='\\u203A';"
      + "var dots=document.createElement('div');dots.className='bk-slider-dots';"
      + "figs.forEach(function(f,i){var d=document.createElement('button');d.type='button';d.setAttribute('aria-label','" + tpl("รูปที่", "Slide") + " '+(i+1));d.addEventListener('click',function(){go(i);});dots.appendChild(d);});"
      + "sl.appendChild(prev);sl.appendChild(next);sl.appendChild(dots);"
      + "function upd(){for(var i=0;i<dots.children.length;i++){dots.children[i].classList.toggle('on',i===cur);}}"
      + "function go(i){cur=Math.max(0,Math.min(figs.length-1,i));track.scrollTo({left:cur*track.clientWidth,behavior:'smooth'});upd();}"
      + "prev.addEventListener('click',function(){go(cur-1);});"
      + "next.addEventListener('click',function(){go(cur+1);});"
      + "var st;track.addEventListener('scroll',function(){if(st)return;st=setTimeout(function(){st=null;cur=Math.round(track.scrollLeft/track.clientWidth);upd();},90);},{passive:true});"
      + "upd();"
      + "});"
      + "}());/*]]>*/<\/script>";
    return css + sc;
  ...
  function storiesStatic(p) {
    ...
    var css = "<style>"
      ...
      + ".bxb-story-ring{display:block;width:70px;height:70px;aspect-ratio:1;flex:none;border-radius:50%;padding:3px;background:linear-gradient(45deg,#f59e0b,#e11d48,#8b5cf6);margin:0 auto;box-sizing:border-box;overflow:hidden}"
      + ".bxb-story-item.seen .bxb-story-ring{background:var(--border-med,#cbd5e1)}"
      + ".bxb-story-ring img{width:100%;height:100%;aspect-ratio:1;max-width:100%;border-radius:50%;object-fit:cover;border:2px solid var(--bg-header,#fff);background:var(--bg-surface-2,#eee);display:block;box-sizing:border-box}"
    ...
    var sc = "<script>/*<![CDATA[*/(function(){"
      ...
      // feed thumbs come back at s72, which is soft on a 70px ring at 2x
      + "function ring(u){return u?u.replace(/\\/s\\d+(-c)?\\//,'/s160-c/').replace(/=s\\d+[^&]*/,'=s160-c'):u;}"
      ...
      + "b.innerHTML='<span class=\"bxb-story-ring\"><img src=\"'+ring(s.thumb)+'\" width=\"160\" height=\"160\" alt=\"\" loading=\"lazy\"/></span><span class=\"bxb-story-cap\">'+ (s.title.replace(/</g,'&lt;')) +'</span>';"
  ...
  function faqStatic() {
    ...
    var sc = "<script>/*<![CDATA[*/(function(){"
      + "var scope=document.querySelector('.bxb-post-article .post-body')||document.querySelector('.post-body');if(!scope)return;"
      + "var items=[].slice.call(scope.querySelectorAll('.bk-faq details'));if(!items.length)return;"
      + "var faqs=[];items.forEach(function(d){var q=d.querySelector('summary');if(!q)return;"
      + "var qt=(q.textContent||'').replace(/\\s+/g,' ').trim();"
      + "var cl=d.cloneNode(true),s=cl.querySelector('summary');if(s)s.parentNode.removeChild(s);"
      + "var at=(cl.textContent||'').replace(/\\s+/g,' ').trim();"
      + "if(qt&&at)faqs.push({'@type':'Question',name:qt,acceptedAnswer:{'@type':'Answer',text:at}});});"
      + "if(!faqs.length)return;"
      + "var sd=document.createElement('script');sd.type='application/ld+json';"
      + "sd.text=JSON.stringify({'@context':'https://schema.org','@type':'FAQPage',mainEntity:faqs});"
  ...
  var SHARE_MARKS = {
    ...
    lock: "<path d='M18 8h-1V6c0-2.76-2.24-5-5-5S7 3.24 7 6v2H6c-1.1 0-2 .9-2 2v10c0 1.1.9 2 2 2h12c1.1 0 2-.9 2-2V10c0-1.1-.9-2-2-2zM9 6c0-1.66 1.34-3 3-3s3 1.34 3 3v2H9V6zm3 12c-1.1 0-2-.9-2-2s.9-2 2-2 2 .9 2 2-.9 2-2 2z'/>"
  };
  ...
  // The card markup and its behaviour are emitted separately: with pos "both" there are
  // two cards on the page and only one handler, which binds every [data-share] it finds.
  function shareCardHtml(p, where) {
...
"<script>/*<![CDATA[*/(function(){" +
  ...
  var LIB = [
    ...
    ["บทความ & UX", [
      ...
      ["backtotop", "ปุ่มกลับขึ้นบน", "ปุ่มลอยเลื่อนขึ้นบนสุด"],
  ...
  function blockDefaults(type, lang) {
    ...
    var d = isEn ? {
      ...
      categories: [], catLabel: "Categories", catFooter: true, catFooterMax: 12 },
      ...
      share: { label: "Share this post", pos: "bottom", facebook: true, twitter: true, line: true, threads: true, pinterest: true, copy: true },
      ...
      faq: {},
      slider: {},
      sharebar: { side: "left", facebook: true, twitter: true, line: true, copy: true },
      ...
      categories: [], catLabel: "หมวดหมู่สินค้า", catFooter: true, catFooterMax: 12 },
      ...
      share: { label: "แชร์บทความนี้", pos: "bottom", facebook: true, twitter: true, line: true, threads: true, pinterest: true, copy: true },
      sidebar: { position: "right", width: "280px", showSearch: true, showCategories: true, showArchive: true, showAbout: false },
    ...
    // Label links are used because they resolve on a brand-new blog; page links would 404 until
    // the user creates the pages, and the props panel nudges them to switch once they have.
    if (type === "header" && tid === "review") {
      var seed = en
  ...
  /* ---------- Product categories (review template · hand-entered, plain HTML) ---------- */
  function catsOf(p) { return Array.isArray(p.categories) ? p.categories : []; }
  // one category row -> a real URL. Page links win on SEO, label links work with zero setup.
  function catUrlFor(c) {
    var t = c.linkType || "label";
    if (t === "label") {
      var ln = (c.labelName || "").trim();
      return ln ? "/search/label/" + encodeURIComponent(ln) : "";
    }
    if (t === "page") {
      var raw = (c.pageSlug || "").trim();
      if (!raw) return "";
      if (/^\/p\/.+\.html$/.test(raw)) return raw;
      return pageNameToUrl(raw) || "";
    }
    return (c.url || "").trim();
  }
  function headerBlockOf() {
    var bs = (S && S.blocks) || [];
    for (var i = 0; i < bs.length; i++) { if (bs[i].type === "header") return bs[i]; }
    return null;
  }
  // categories only exist on the review template, and only once the user has filled them in
  function reviewCats() {
    if (!S || S.templateId !== "review") return [];
    var hb = headerBlockOf();
    if (!hb) return [];
    return catsOf(hb.props).filter(function (c) { return (c.label || "").trim() && catUrlFor(c); });
  }
  function catMenuLabel() {
    var hb = headerBlockOf();
    var v = hb && hb.props.catLabel;
    return (v != null && v !== "") ? v : tpl("หมวดหมู่สินค้า", "Categories");
  }
  // 1 column up to 8 items, 2 up to 16, 3 beyond · keeps the small case looking like a plain menu
  function catCols(n) { return n <= 8 ? 1 : (n <= 16 ? 2 : 3); }
  var CAT_FILTER_MIN = 16; // show the type-to-filter box only once the list gets long
  // Shared <li> markup for the desktop dropdown and the mobile sheet, so one filter handles both.
  // Each row is its own SiteNavigationElement rather than one scope wrapping every name.
  function catItemsHtml(cats) {
  ...
  function catSearchHtml(n, id) {
    if (n < CAT_FILTER_MIN) return "";
    return "<div class='rv-cat-search'>" +
      ...
      "<input type='search' id='" + id + "' class='rv-cat-filter' autocomplete='off' placeholder='" + tpl("พิมพ์เพื่อกรองหมวดหมู่…", "Type to filter categories…") + "' aria-label='" + tpl("กรองหมวดหมู่", "Filter categories") + "'/>" +
      "</div>";
  }
  ...
  // shared dark-mode controller · every toggle (Dark Mode block, bottom-nav Mode) uses this so they stay in sync
  var THEME_CTRL_JS = "window.__bxbTheme=window.__bxbTheme||(function(){var K='bxb-theme',subs=[];function cur(){return document.documentElement.dataset.theme==='dark'?'dark':'light';}function apply(t){document.documentElement.dataset.theme=t;try{localStorage.setItem(K,t);}catch(e){}subs.forEach(function(f){try{f(t);}catch(e){}});}var init=null;try{init=localStorage.getItem(K);}catch(e){}if(!document.documentElement.dataset.theme){document.documentElement.dataset.theme=init||(matchMedia('(prefers-color-scheme:dark)').matches?'dark':'light');}return{get:cur,set:apply,toggle:function(){apply(cur()==='dark'?'light':'dark');},sub:function(f){subs.push(f);try{f(cur());}catch(e){}}};})();";
  // Type-to-filter for the category dropdown and sheet. Defined once, called with either root.
  var CAT_FILTER_JS = "window.__bxbCatFilter=window.__bxbCatFilter||function(root){"
    + "var inp=root.querySelector('.rv-cat-filter');if(!inp)return;"
    + "var lis=[].slice.call(root.querySelectorAll('.rv-cat-list>li'));"
    + "var none=root.querySelector('.rv-cat-none');"
    + "inp.addEventListener('input',function(){"
    + "var q=inp.value.trim().toLowerCase(),hit=0;"
    + "lis.forEach(function(li){"
    + "var n=li.querySelector('.rv-cat-name');"
    + "var ok=!q||(n&&n.textContent.toLowerCase().indexOf(q)>=0);"
    + "li.hidden=!ok;if(ok)hit++;});"
    + "if(none)none.hidden=hit>0;});"
    + "inp.addEventListener('keydown',function(e){if(e.key==='Escape'){e.stopPropagation();inp.value='';inp.dispatchEvent(new Event('input'));}});"
    + "};";
  var BOTNAV_ICONS = {
  ...
  function botNavLabels() {
    return { home: tpl("หน้าแรก", "Home"), search: tpl("ค้นหา", "Search"), menu: tpl("เมนู", "Menu"), mode: tpl("โหมด", "Mode"), share: tpl("แชร์", "Share"), cats: tpl("หมวดหมู่", "Categories") };
  }
  var CAT_GRID_ICON = '<rect x="3" y="3" width="7.5" height="7.5" rx="2"/><rect x="13.5" y="3" width="7.5" height="7.5" rx="2"/><rect x="3" y="13.5" width="7.5" height="7.5" rx="2"/><rect x="13.5" y="13.5" width="7.5" height="7.5" rx="2"/>';
  ...
  function botNavStatic() {
    ...
    var css = "<style>"
      ...
      + ".bxb-botnav{display:flex;position:fixed;left:0;right:0;bottom:0;z-index:200;background:var(--bg-header,#fff);border-top:1px solid var(--border,#e8eaf2);padding:6px 4px calc(6px + env(safe-area-inset-bottom,0px));box-shadow:0 -4px 20px rgba(0,0,0,.07);backdrop-filter:blur(10px);animation:bxbBnUp .5s cubic-bezier(.22,1,.36,1) .15s both}"
      + "@keyframes bxbBnUp{from{transform:translateY(100%)}to{transform:none}}"
      + "@keyframes bxbBnPop{0%{transform:scale(1)}45%{transform:scale(1.22)}100%{transform:scale(1)}}"
      + "@media(prefers-reduced-motion:reduce){.bxb-botnav{animation:none}.bxb-bn:active .bn-ic,.bxb-bn:focus-visible .bn-ic{animation:none}}"
      ...
      + ".bxb-bn .bn-ic{transition:transform .2s cubic-bezier(.34,1.56,.64,1)}"
      + ".bxb-bn:active .bn-ic,.bxb-bn:focus-visible .bn-ic{animation:bxbBnPop .35s cubic-bezier(.34,1.56,.64,1)}"
      + ".bxb-bn:hover .bn-ic{transform:translateY(-2px)}"
    ...
    var pop = "<div class='bxb-sp-scrim' id='bxbSpScrim'></div>"
      ...
      + "</form></div>";
    // Slot 3 becomes Categories on the review template. The site menu (Home / About / Contact)
    // still lives in the top-right hamburger, so nothing is lost by swapping it out.
    var bnCats = reviewCats();
    var slot3 = bnCats.length
      ? "<button type='button' class='bxb-bn' id='bxbCatBtn' aria-haspopup='dialog' aria-expanded='false' aria-controls='rvCatSheet'>" + bi(CAT_GRID_ICON) + "<span>" + L.cats + "</span></button>"
    ...
    var bar = "<nav class='bxb-botnav' aria-label='" + tpl("เมนูล่าง", "Bottom menu") + "'>"
      ...
      + "</nav>";
    var sheet = bnCats.length
      ? "<div class='rv-cat-scrim' id='rvCatScrim'></div>"
        + "<div class='rv-cat-sheet' id='rvCatSheet' role='dialog' aria-modal='true' aria-labelledby='rvCatSheetTitle'>"
        + "<div class='rv-cat-sheet-head'>"
        + "<h3 id='rvCatSheetTitle'>" + esc(catMenuLabel()) + "</h3>"
        + "<button type='button' class='rv-cat-x' id='rvCatSheetX' aria-label='" + tpl("ปิด", "Close") + "'>✕</button>"
        + "</div>"
        + catSearchHtml(bnCats.length, "rvCatFilterM")
        + "<ul class='rv-cat-list' role='menu'>" + catItemsHtml(bnCats) + "</ul>"
        + "<p class='rv-cat-none' hidden='hidden'>" + tpl("ไม่พบหมวดหมู่ที่ค้นหา", "No matching category") + "</p>"
        + "</div>"
      : "";
    var sc = "<script>/*<![CDATA[*/(function(){"
      + THEME_CTRL_JS
      + "var TC=window.__bxbTheme;"
      + "var MOON='<path d=\"M21 12.8A8.5 8.5 0 1 1 11.2 3a6.5 6.5 0 0 0 9.8 9.8z\"/>';"
      + "var SUN='<circle cx=\"12\" cy=\"12\" r=\"4\"/><path d=\"M12 2v2M12 20v2M4 12H2M22 12h-2M5.6 5.6l1.4 1.4M17 17l1.4 1.4M5.6 18.4 7 17M17 7l1.4-1.4\"/>';"
      + "var m=document.getElementById('bxbModeBtn');"
      + "if(m){m.addEventListener('click',function(){TC.toggle();});TC.sub(function(t){var ic=m.querySelector('.bn-ic');if(ic)ic.innerHTML=(t==='dark'?SUN:MOON);});}"
      ...
      + (bnCats.length ? CAT_FILTER_JS
        + "var sh=document.getElementById('rvCatSheet'),shb=document.getElementById('bxbCatBtn'),"
        + "shx=document.getElementById('rvCatSheetX'),shs=document.getElementById('rvCatScrim');"
        + "if(sh&&shb){"
        + "function shSet(o){sh.classList.toggle('open',o);if(shs)shs.classList.toggle('open',o);"
        + "shb.setAttribute('aria-expanded',o?'true':'false');document.body.classList.toggle('rv-cat-lock',o);"
        + "if(o){var f=sh.querySelector('.rv-cat-filter');if(f)setTimeout(function(){f.focus();},300);}}"
        + "shb.addEventListener('click',function(){shSet(!sh.classList.contains('open'));});"
        + "if(shx)shx.addEventListener('click',function(){shSet(false);});"
        + "if(shs)shs.addEventListener('click',function(){shSet(false);});"
        + "document.addEventListener('keydown',function(e){if(e.key==='Escape')shSet(false);});"
        + "window.__bxbCatFilter(sh);"
        + "}" : "")
      + "}());/*]]>*/<\/script>";
    return css + pop + bar + sheet + sc;
  }
  ...
  function botNavPreview() {
    ...
    var pvSlot3 = reviewCats().length ? item(CAT_GRID_ICON, L.cats) : item(BOTNAV_ICONS.menu, L.menu);
    return '<div style="position:sticky;bottom:0;display:flex;background:#fff;border-top:1px solid #e8eaf2;box-shadow:0 -4px 20px rgba(0,0,0,.07);padding:8px 4px 10px">' +
      item(BOTNAV_ICONS.home, L.home) + item(BOTNAV_ICONS.search, L.search) + pvSlot3 + item(BOTNAV_ICONS.mode, L.mode) + item(BOTNAV_ICONS.share, L.share) +
      '</div>';
  }
  var PAGE_CHECKLIST = [
  ...
  function renderBlockInner(b) {
    ...
    // preview mirrors of the exported --primary-ink / --accent-ink, so the canvas shows the
    // same contrast-corrected fills the theme will actually ship
    var prI = inkFor(pr), acI = inkFor(ac);
    switch (b.type) {
      case "header":
        ...
        // mirror the exported category dropdown so the canvas shows what ships
        var pvCats = reviewCats();
        if (pvCats.length && !isMob) {
          ...
          var pvMore = pvCats.length > pvCols * 4
            ? '<div style="grid-column:1/-1;text-align:center;font-size:11.5px;color:#9aa;padding:6px 0 2px">' + tpl("และอีก " + (pvCats.length - pvCols * 4) + " หมวด", "+" + (pvCats.length - pvCols * 4) + " more") + '</div>'
            : "";
          menu.splice(menu.length > 1 ? 1 : menu.length, 0,
            '<div style="position:relative;display:inline-block">' +
              '<a style="color:' + pr + ';font-weight:600;font-size:15px;text-decoration:none;cursor:pointer;display:inline-flex;align-items:center;gap:7px;background:#f4f5fb;padding:7px 13px;border-radius:8px">' +
                '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9" style="width:16px;height:16px"><rect x="3" y="3" width="7.5" height="7.5" rx="2"/><rect x="13.5" y="3" width="7.5" height="7.5" rx="2"/><rect x="3" y="13.5" width="7.5" height="7.5" rx="2"/><rect x="13.5" y="13.5" width="7.5" height="7.5" rx="2"/></svg>' +
                esc(catMenuLabel()) + ' <span style="font-size:10px">▾</span></a>' +
              '<div style="position:absolute;top:calc(100% + 8px);left:50%;transform:translateX(-50%);background:#fff;border:1px solid #eef;border-radius:14px;box-shadow:0 10px 28px rgba(0,0,0,.13);padding:8px;z-index:3;display:grid;grid-template-columns:repeat(' + pvCols + ',minmax(180px,1fr));gap:2px 6px">' + pvRows + pvMore + '</div>' +
            '</div>');
        }
        menu = menu.join("");
      ...
      case "hero":
        ...
        if (S && S.templateId === "review") {
          ...
          return '<div style="position:relative;overflow:hidden;padding:80px 32px;background:radial-gradient(58% 80% at 84% 8%,rgba(255,255,255,.28),transparent 60%),linear-gradient(135deg,' + prI + ',' + acI + ');color:#fff">' +
        ...
        if (S && S.templateId === "course") {
          ...
          return '<div style="position:relative;overflow:hidden;padding:80px 32px;background:radial-gradient(circle at 12% 18%,rgba(255,255,255,.17),transparent 26%),radial-gradient(circle at 88% 84%,rgba(255,255,255,.13),transparent 24%),linear-gradient(135deg,' + prI + ',' + acI + ');color:#fff">' +
            '<div style="position:relative;max-width:860px;margin:0 auto">' +
        ...
        if (S && S.templateId === "company") {
          return '<div style="padding:80px 32px;background:linear-gradient(135deg,' + prI + ',' + acI + ');color:#fff;position:relative;overflow:hidden">' +
        ...
        if (S && S.templateId === "personal") {
          ...
          return '<div style="position:relative;overflow:hidden;padding:84px 32px 74px;background:radial-gradient(circle,' + hexRgba(pr, ".14") + ' 1.5px,transparent 1.9px) 0 0/24px 24px,radial-gradient(640px 300px at 18% -8%,' + hexRgba(pr, ".09") + ',transparent 70%),#fffdf8;border-bottom:1px solid #f0ece2">' +
            '<div aria-hidden="true" style="position:absolute;top:-70px;right:-70px;width:280px;height:280px;border-radius:50%;border:2px dashed ' + hexRgba(pr, ".35") + '"></div>' +
            '<div aria-hidden="true" style="position:absolute;left:-40px;bottom:-40px;width:180px;height:180px;border-radius:50%;background:radial-gradient(circle at 35% 35%,' + hexRgba(ac, ".22") + ',transparent 68%)"></div>' +
            '<div style="position:relative;max-width:860px;margin:0 auto;display:flex;align-items:center;gap:48px;flex-wrap:wrap">' +
              '<div style="flex:1;min-width:220px">' +
                (pbHeroEb ? '<div style="display:inline-block;font-size:11px;color:#fff;background:linear-gradient(120deg,' + prI + ',' + acI + ');font-weight:700;letter-spacing:.18em;text-transform:uppercase;padding:7px 16px;border-radius:999px;transform:rotate(-2deg);box-shadow:0 6px 16px -6px ' + hexRgba(pr, ".55") + ';margin-bottom:18px">' + esc(pbHeroEb) + '</div>' : '') +
              ...
              (p.showImage !== false
                ? '<div style="flex:none">' +
                    '<div style="width:150px;height:150px;border-radius:50%;padding:5px;background:conic-gradient(from 210deg,' + pr + ',' + ac + ',' + pr + ');box-shadow:0 18px 44px -16px ' + hexRgba(pr, ".45") + '">' +
                      (p.imageUrl
        ...
        if (S && S.templateId === "review") {
          ...
          for (var rvi = 0; rvi < (p.count || 6); rvi++) {
            ...
            rvCards += '<article style="border-radius:' + r + ';overflow:hidden;background:#fff;box-shadow:0 2px 12px rgba(0,0,0,.08)">' +
              '<div style="aspect-ratio:16/9;overflow:hidden;background:linear-gradient(135deg,' + (rvi % 2 === 0 ? pr + ',' + ac : ac + ',' + pr) + ');position:relative">' +
                '<span style="position:absolute;top:17px;left:-38px;width:150px;transform:rotate(-45deg);text-align:center;font-size:10px;font-weight:800;letter-spacing:.07em;line-height:1.35;text-transform:uppercase;padding:6px 0;color:#fff;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;background:linear-gradient(135deg,' + pr + '40,' + ac + '26);-webkit-backdrop-filter:saturate(1.25) brightness(.86);backdrop-filter:saturate(1.25) brightness(.86);box-shadow:inset 0 1px 0 rgba(255,255,255,.4),inset 0 -1px 0 rgba(255,255,255,.22);text-shadow:0 1px 3px rgba(0,0,0,.62),0 0 9px rgba(0,0,0,.42)">' + rvCat + '</span>' +
              ...
              '<div style="border-top:1px solid #f0f0f8;padding:11px 17px;display:flex;justify-content:space-between;align-items:center">' +
                ...
                '<a style="display:inline-flex;align-items:center;gap:6px;font-size:13px;font-weight:700;color:#fff;background:linear-gradient(135deg,' + prI + ',' + acI + ');padding:9px 19px;border-radius:99px;text-decoration:none;white-space:nowrap;box-shadow:0 3px 10px ' + pr + '3d">' + tpl("ดูดีลเลย →", "See Deal →") + '</a>' +
              '</div>' +
            '</article>';
          }
      ...
      case "cta":
        if (S && S.templateId === "course") {
          ...
          return '<div style="padding:84px 32px;background:linear-gradient(135deg,' + prI + ',' + acI + ');text-align:center;color:#fff">' +
      ...
      case "share":
        ...
        var shPos = p.pos || "bottom";
        var shWhere = shPos === "top" ? tpl("บนบทความ", "Top of the post")
          : shPos === "both" ? tpl("บนและล่างบทความ", "Top and bottom of the post")
          : tpl("ใต้บทความ ก่อนความคิดเห็น", "Under the post, above the comments");
      ...
      case "callout":
        ...
        return '<div style="padding:18px 32px">' +
          ...
          '</div>';
      case "proscons":
        var pcLi = function (txt, ok) { return '<li style="position:relative;padding-left:26px;margin:6px 0;line-height:1.5">' +
          '<span style="position:absolute;left:0;top:1px;color:' + (ok ? "#16a34a" : "#dc2626") + ';font-weight:800">' + (ok ? "✓" : "✕") + '</span>' + txt + '</li>'; };
        return '<div style="padding:18px 32px">' +
          '<div style="font-size:11px;font-weight:700;color:#828aa0;text-transform:uppercase;letter-spacing:.06em;margin-bottom:12px">' + tpl("Pros / Cons + คะแนนรีวิว · ใช้ในเนื้อหาบทความ", "Pros / Cons + verdict · for post content") + '</div>' +
          '<div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;color:#1e2333;font-size:13.5px">' +
            '<div style="border-radius:12px;padding:13px 16px;background:' + hexRgba("#16a34a", ".08") + ';border:1px solid ' + hexRgba("#16a34a", ".28") + '"><b style="color:#16a34a;display:block;margin-bottom:6px">' + tpl("ข้อดี", "Pros") + '</b><ul style="margin:0;padding:0;list-style:none">' + pcLi(tpl("ใช้งานง่าย", "Easy to use"), true) + pcLi(tpl("คุ้มราคา", "Great value"), true) + '</ul></div>' +
            '<div style="border-radius:12px;padding:13px 16px;background:' + hexRgba("#dc2626", ".08") + ';border:1px solid ' + hexRgba("#dc2626", ".28") + '"><b style="color:#dc2626;display:block;margin-bottom:6px">' + tpl("ข้อเสีย", "Cons") + '</b><ul style="margin:0;padding:0;list-style:none">' + pcLi(tpl("แบตหมดไว", "Short battery"), false) + '</ul></div>' +
          '</div>' +
          '<div style="display:flex;gap:16px;align-items:center;border:1px solid #e8eaf2;border-left:5px solid ' + pr + ';border-radius:12px;padding:14px 18px;margin-top:14px;background:#f7f8fc">' +
            '<div style="font-size:2.3rem;font-weight:800;color:' + pr + ';line-height:1">4.5</div>' +
            '<div><b style="font-size:15px;color:#1e2333">' + tpl("ดีเยี่ยม", "Excellent") + '</b><div style="color:#f59e0b;letter-spacing:2px;font-size:14px">★★★★★</div><p style="margin:4px 0 0;font-size:12.5px;color:#828aa0">' + tpl("สรุปรีวิวสั้นๆ ของสินค้า", "A short verdict about the product") + '</p></div>' +
          '</div>' +
          ...
          '</div>';
      case "faq":
        var faqRow = function (q, open) { return '<div style="border:1px solid ' + (open ? pr : "#e8eaf2") + ';border-radius:12px;background:#fff;overflow:hidden">' +
          '<div style="display:flex;align-items:center;justify-content:space-between;padding:13px 16px;font-weight:600;font-size:14px;color:' + (open ? pr : "#1e2333") + '"><span>' + q + '</span>' +
          '<span style="width:9px;height:9px;border-right:2px solid ' + (open ? pr : "#64748b") + ';border-bottom:2px solid ' + (open ? pr : "#64748b") + ';transform:rotate(' + (open ? "-135deg" : "45deg") + ');display:inline-block;flex:none"></span></div>' +
          (open ? '<div style="padding:0 16px 15px;font-size:13px;color:#4a5063;line-height:1.65">' + tpl("คำตอบแบบละเอียด อธิบายให้ผู้อ่านเข้าใจง่าย และช่วยให้ AI/Google ดึงไปตอบคำถามได้", "A clear, detailed answer that helps readers — and lets AI/Google pull it as an answer") + '</div>' : "") + '</div>'; };
        return '<div style="padding:18px 32px">' +
          '<div style="font-size:11px;font-weight:700;color:#828aa0;text-transform:uppercase;letter-spacing:.06em;margin-bottom:12px">' + tpl("คำถามที่พบบ่อย (FAQ) · ใช้ในเนื้อหาบทความ", "FAQ accordion · for post content") + '</div>' +
      ...
      case "morefrom":
        ...
        var mfPCols = Math.min(mfP.columns || 3, 3);
        var mfCard = '<div style="border:1px solid #eef;border-radius:10px;overflow:hidden;background:#fff"><div style="aspect-ratio:16/9;background:linear-gradient(135deg,#e8eaf5,#dfe3f0)"></div><div style="padding:10px 12px"><div style="height:8px;background:#e2e5f0;border-radius:4px;margin-bottom:6px"></div><div style="height:8px;width:60%;background:#eceef6;border-radius:4px"></div></div></div>';
        return '<div style="padding:18px 32px">' +
          '<div style="font-size:11px;font-weight:700;color:#828aa0;text-transform:uppercase;letter-spacing:.06em;margin-bottom:12px">' + (mfP.mode === "random" ? tpl("สุ่มบทความ", "Random posts") : tpl("บทความล่าสุด", "Recent posts")) + (mfP.label ? ' · ' + esc(mfP.label) : "") + '</div>' +
          '<h3 style="font-size:18px;font-weight:700;margin:0 0 14px;color:#1e2333">' + mfPHeading + '</h3>' +
          '<div style="display:grid;grid-template-columns:repeat(' + mfPCols + ',1fr);gap:12px">' + new Array(mfPCols).fill(mfCard).join("") + '</div>' +
          '<div style="margin-top:11px;font-size:12px;color:#828aa0;line-height:1.6">' + tpl("ดึงบทความจาก Feed อัตโนมัติ (สุ่ม หรือ ล่าสุด) · กรองตามป้ายกำกับได้ · ทำงานบนบล็อกจริง", "Auto-fetches posts from the Feed (random or recent) · filter by label · works on the live blog") + '</div>' +
          '</div>';
      case "slider":
        return '<div style="padding:18px 32px">' +
          '<div style="font-size:11px;font-weight:700;color:#828aa0;text-transform:uppercase;letter-spacing:.06em;margin-bottom:12px">' + tpl("สไลด์รูป/แกลเลอรี · ใช้ในเนื้อหาบทความ", "Image slider/gallery · for post content") + '</div>' +
          '<div style="position:relative;border-radius:12px;overflow:hidden;background:#0b0f1a;aspect-ratio:16/9;display:flex;align-items:flex-end">' +
            '<div style="position:absolute;inset:0;background:linear-gradient(135deg,#334155,#1e293b)"></div>' +
            '<div style="position:absolute;top:50%;left:8px;transform:translateY(-50%);width:32px;height:32px;border-radius:50%;background:rgba(0,0,0,.4);color:#fff;display:grid;place-items:center;font-size:17px">‹</div>' +
            '<div style="position:absolute;top:50%;right:8px;transform:translateY(-50%);width:32px;height:32px;border-radius:50%;background:rgba(0,0,0,.4);color:#fff;display:grid;place-items:center;font-size:17px">›</div>' +
            '<div style="position:relative;z-index:1;color:#fff;padding:22px 14px 30px;font-size:12.5px;background:linear-gradient(transparent,rgba(0,0,0,.7));width:100%">' + tpl("คำบรรยายใต้ภาพที่ 1", "Caption for image 1") + '</div>' +
            '<div style="position:absolute;bottom:10px;left:50%;transform:translateX(-50%);z-index:2;display:flex;gap:5px"><span style="width:16px;height:6px;border-radius:3px;background:#fff"></span><span style="width:6px;height:6px;border-radius:50%;background:rgba(255,255,255,.5)"></span><span style="width:6px;height:6px;border-radius:50%;background:rgba(255,255,255,.5)"></span></div>' +
          '</div>' +
          '<div style="margin-top:11px;font-size:12px;color:#828aa0;line-height:1.6">' + tpl("คัดลอกโค้ดจากแผงด้านขวา ใส่ลิงก์รูปของคุณ · เลื่อนซ้าย-ขวาได้ พร้อมลูกศรและจุดบอกตำแหน่ง", "Copy the snippet from the right panel and add your image URLs · swipeable with arrows and dots") + '</div>' +
          '</div>';
      case "sharebar":
        var shP = b.props || {};
        var shDot = function (c) { return '<span style="width:38px;height:38px;border-radius:11px;background:' + c + ';display:grid;place-items:center;box-shadow:0 2px 8px rgba(0,0,0,.15)"><span style="width:15px;height:15px;border-radius:3px;background:rgba(255,255,255,.9)"></span></span>'; };
        var shList = [shP.facebook !== false ? "#1877f2" : null, shP.twitter !== false ? "#000" : null, shP.line !== false ? "#06c755" : null, shP.copy !== false ? (pr) : null].filter(Boolean);
        return '<div style="padding:18px 32px">' +
          '<div style="font-size:11px;font-weight:700;color:#828aa0;text-transform:uppercase;letter-spacing:.06em;margin-bottom:12px">' + tpl("แถบแชร์ลอยข้างจอ · เฉพาะหน้าบทความ (เดสก์ท็อป)", "Sticky share bar · post pages only (desktop)") + '</div>' +
      ...
      case "backtotop":
        ...
        return '<div style="position:relative;height:120px;border-radius:12px;background:#f7f8fc;border:1px solid #eef;overflow:hidden">' +
          '<div style="position:absolute;bottom:14px;' + (bttLeft ? "left:14px" : "right:14px") + ';width:44px;height:44px;border-radius:50%;background:' + pr + ';display:grid;place-items:center;box-shadow:0 4px 16px rgba(0,0,0,.22)">' +
          '<svg viewBox="0 0 24 24" fill="none" stroke="#fff" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round" style="width:20px;height:20px"><path d="M12 19V5M5 12l7-7 7 7"/></svg></div>' +
          '<div style="padding:14px 16px;font-size:12.5px;color:#828aa0;line-height:1.6">' + tpl("ปุ่มลอยกลับขึ้นบนสุด · แสดงเมื่อเลื่อนลงมา แล้วเลื่อนขึ้นแบบนุ่มนวล", "A floating button that appears after scrolling and smooth-scrolls to top") + '</div>' +
          '</div>';
      case "themepicker":
      ...
      case "lightbox":
        return '<div style="padding:18px 32px">' +
          '<div style="font-size:11px;font-weight:700;color:#828aa0;text-transform:uppercase;letter-spacing:.06em;margin-bottom:12px">' + tpl("ซูมรูปภาพ (Lightbox)", "Image Lightbox") + '</div>' +
          '<div style="position:relative;max-width:300px;border-radius:12px;overflow:hidden;background:#0b0b12;display:grid;place-items:center;aspect-ratio:16/10">' +
            '<div style="width:62%;aspect-ratio:16/10;border-radius:8px;background:linear-gradient(120deg,#c7cbe0,#e8eaf2);box-shadow:0 8px 30px rgba(0,0,0,.5)"></div>' +
            '<div style="position:absolute;top:10px;right:12px;width:30px;height:30px;border-radius:50%;background:rgba(255,255,255,.16);color:#fff;display:grid;place-items:center;font-size:15px">✕</div>' +
            '<div style="position:absolute;left:8px;top:50%;transform:translateY(-50%);width:30px;height:30px;border-radius:50%;background:rgba(255,255,255,.16);color:#fff;display:grid;place-items:center;font-size:18px">‹</div>' +
            '<div style="position:absolute;right:8px;top:50%;transform:translateY(-50%);width:30px;height:30px;border-radius:50%;background:rgba(255,255,255,.16);color:#fff;display:grid;place-items:center;font-size:18px">›</div>' +
          '</div>' +
      ...
      case "copycode":
        return '<div style="padding:18px 32px">' +
          ...
          '<div style="position:relative;background:#0d1117;border-radius:10px;padding:14px 16px;font-family:ui-monospace,monospace;font-size:12.5px;color:#c9d1d9;line-height:1.7;overflow:hidden">' +
            '<button style="position:absolute;top:8px;right:8px;display:inline-flex;align-items:center;gap:5px;border:1px solid rgba(255,255,255,.18);background:rgba(255,255,255,.08);color:#e9eaf2;font-size:12px;padding:5px 10px;border-radius:8px"><svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="9" y="9" width="11" height="11" rx="2"/><path d="M5 15V5a2 2 0 0 1 2-2h8"/></svg>' + tpl("คัดลอก", "Copy") + '</button>' +
            '<div style="color:#7ee787">const</div><div>&nbsp;&nbsp;greet = () =&gt; console.log(&quot;hi&quot;);</div>' +
          '</div>' +
      ...
      case "footer":
        ...
        var pvFHb = headerBlockOf();
        var pvFCats = (pvFHb && pvFHb.props.catFooter === false) ? [] : reviewCats();
        var pvFMax = parseInt(pvFHb && pvFHb.props.catFooterMax != null ? pvFHb.props.catFooterMax : 12, 10);
        if (pvFMax > 0) pvFCats = pvFCats.slice(0, pvFMax);
        var pvFCatsHtml = pvFCats.length
          ? '<div><div style="display:flex;align-items:center;gap:8px;font-size:12.5px;font-weight:700;color:' + fFg + ';margin-bottom:12px">' +
        ...
        var fGridStyle = VIEW === "mobile"
          ...
          : "display:grid;grid-template-columns:" + (pvFCatsHtml ? "1.4fr .8fr 1.3fr" : "1.5fr 1fr") + ";gap:40px;align-items:start";
        ...
        return '<div style="background:' + fBg + ';color:' + fFg + ';padding:48px 32px 24px;border-top:3px solid ' + d.primary + '">' +
  ...
  var SINGLETON_BLOCKS = { share: 1, toc: 1, darkmode: 1, notfound: 1, progress: 1, sidebar: 1, header: 1, footer: 1, aeo: 1, related: 1, callout: 1, readtime: 1, bookmark: 1, lightbox: 1, copycode: 1, dropcap: 1, anchorlink: 1, proscons: 1, affiliate: 1, faq: 1, slider: 1, sharebar: 1, breadcrumb: 1, backtotop: 1, announce: 1, cookie: 1, morefrom: 1, stories: 1, translate: 1, themepicker: 1 };
  ...
  // Category editor · review template only. Hand-entered so the user controls anchor text,
  // link target and ordering, which is where the real internal-linking value sits.
  function categoriesEditor(p) {
    var cats = catsOf(p);
    var usesLabel = false;
    var rows = cats.map(function (c, i) {
      var t = c.linkType || "label";
      var url = catUrlFor(c);
      if (t === "label") usesLabel = true;
      var typeSel = '<select class="inp" data-catt="' + i + '" style="flex:0 0 auto;width:104px;padding:6px 4px;font-size:12px">' +
        '<option value="page"' + (t === "page" ? " selected" : "") + '>' + tpl("หน้าเพจ", "Page") + '</option>' +
        '<option value="label"' + (t === "label" ? " selected" : "") + '>' + tpl("ป้ายกำกับ", "Label") + '</option>' +
        '<option value="url"' + (t === "url" ? " selected" : "") + '>' + tpl("URL เอง", "Custom URL") + '</option>' +
        '</select>';
      var val = t === "page" ? (c.pageSlug || "") : t === "label" ? (c.labelName || "") : (c.url || "");
      var ph = t === "page" ? tpl("ชื่อหน้า เช่น electronics", "Page slug, e.g. electronics")
             : t === "label" ? tpl("ชื่อป้ายกำกับให้ตรงเป๊ะ", "Exact label name")
             : "https://…";
      return '<div class="menu-row" data-cati="' + i + '">' +
        '<div class="menu-row-top">' +
        ...
        '<div class="menu-secondary" style="display:flex;gap:6px;align-items:center;flex-wrap:wrap">' + typeSel +
          '<input class="inp" data-catv="' + i + '" value="' + esc(val) + '" placeholder="' + ph + '" style="flex:1;min-width:130px">' +
          '<div class="url-chip' + (url ? "" : " empty") + '" style="flex-basis:100%">' + (url ? esc(url) : tpl("ยังไม่มีลิงก์ · หมวดนี้จะถูกข้าม", "No link yet, this row is skipped")) + '</div>' +
        '</div>' +
      '</div>';
    }).join("");
    var n = cats.length;
    return '<div class="field"><label>' + tr("หมวดหมู่สินค้า") + '</label>' +
      '<div class="menu-list">' + rows + '</div>' +
  ...
  function fieldsFor(b) {
    ...
    switch (b.type) {
      case "header": return imgUrlSeo("logoUrl", "URL รูปโลโก้ (Header & Footer & Schema)", S.seo && S.seo.logoUrl || "")
        ...
        + menuEditor(p)
        + (S && S.templateId === "review" ? categoriesEditor(p) : "")
        + tog("showMenuIcons", "แสดงไอคอนหน้าเมนู", p.showMenuIcons !== false) + seg("mobileSide", "เมนูมือถือเด้งจาก", p.mobileSide || "right", [["left", "◧ ซ้าย"], ["right", "ขวา ◨"]]) + tog("sticky", "ติดด้านบน (Sticky)", p.sticky) + tog("showSearch", "แสดงปุ่มค้นหา", p.showSearch)
        + tog("mobileBottomNav", "แถบเมนูล่าง (มือถือ)", p.mobileBottomNav !== false,
            S && S.templateId === "review" && catsOf(p).length
              ? tpl("หน้าแรก · ค้นหา · หมวดหมู่ · โหมด · แชร์", "Home · Search · Categories · Mode · Share")
              : tpl("หน้าแรก · ค้นหา · เมนู · โหมด · แชร์", "Home · Search · Menu · Mode · Share"));
      case "hero": {
      ...
      case "share": return seg("pos", tpl("ตำแหน่ง", "Position"), p.pos || "bottom", [["top", tpl("บน", "Top")], ["bottom", tpl("ล่าง", "Bottom")], ["both", tpl("ทั้งสอง", "Both")]]) + txt("label", tpl("ข้อความนำ", "Lead-in text"), p.label) + tog("facebook", "Facebook", p.facebook) + tog("twitter", "X (Twitter)", p.twitter) + tog("line", "LINE", p.line) + tog("threads", "Threads", p.threads) + tog("pinterest", "Pinterest", p.pinterest) + tog("copy", tpl("คัดลอกลิงก์", "Copy link"), p.copy) + '<div class="note ok">' + svg('<path d="M20 6L9 17l-5-5"/>', 2.5) + '<div>' + tpl("ตำแหน่งถูกล็อกไว้กับหน้าบทความ ลากสลับที่ในผังไม่มีผล · <b>ล่าง</b> อยู่ใต้เนื้อหา ก่อนส่วนแสดงความคิดเห็น · <b>บน</b> อยู่เหนือกล่องสรุปบทความ และอยู่ใต้แถบร้านค้า “เช็คราคาล่าสุดได้ที่” เมื่อใช้เทมเพลตรีวิว", "Locked to the post page; dragging it in the canvas has no effect · <b>Bottom</b> sits under the content, above the comments · <b>Top</b> sits above the summary box, and below the “where to check the price” store bar on the review template") + '</div></div>';
      ...
      case "proscons":
        var pcSnip = '<div class="bk-pc">\n  <div class="bk-pros"><b>' + tpl("ข้อดี", "Pros") + '</b><ul><li>' + tpl("จุดเด่นข้อที่ 1", "Strong point 1") + '</li><li>' + tpl("จุดเด่นข้อที่ 2", "Strong point 2") + '</li></ul></div>\n  <div class="bk-cons"><b>' + tpl("ข้อเสีย", "Cons") + '</b><ul><li>' + tpl("ข้อจำกัดข้อที่ 1", "Drawback 1") + '</li></ul></div>\n</div>';
        var vdSnip = '<div class="bk-verdict"><div class="bk-verdict-score">4.5</div><div class="bk-verdict-body"><b>' + tpl("ดีเยี่ยม", "Excellent") + '</b><div class="bk-stars">★★★★★</div><p>' + tpl("สรุปรีวิวสั้นๆ ของสินค้า/บริการ", "A short verdict about the product/service") + '</p></div></div>';
        var pcBtn = function (label, snip) { return '<div style="display:flex;align-items:center;gap:8px;margin-bottom:8px">' +
          '<code style="flex:1;font-size:11.5px;color:#c7c9e0">' + esc(label) + '</code>' +
          '<button class="callout-copy" data-copy="' + esc(snip) + '" style="flex:none;border:1px solid rgba(255,255,255,.16);background:rgba(255,255,255,.06);color:#e9eaf2;border-radius:7px;padding:5px 11px;font-size:12px;cursor:pointer">' + tpl("คัดลอก", "Copy") + '</button></div>'; };
        return '<div class="note info">' + svg('<circle cx="12" cy="12" r="9"/><path d="M12 8h.01M11 12h1v4h1"/>', 2) + '<div>' + tpl('กล่องข้อดี-ข้อเสีย และกล่องสรุปคะแนน สำหรับบทความรีวิว · กด “คัดลอก” แล้ววางใน Blogger → เขียนโพสต์ → มุมมอง HTML', 'Pros/Cons boxes and a verdict score box for review posts · click “Copy”, then paste in Blogger → Post editor → HTML view') + '</div></div>'
          + '<div class="field"><label>' + tpl("คัดลอกโค้ด", "Copy a snippet") + '</label>' + pcBtn(tpl("ข้อดี / ข้อเสีย (2 คอลัมน์)", "Pros / Cons (2 columns)"), pcSnip) + pcBtn(tpl("กล่องสรุปคะแนน", "Verdict score box"), vdSnip) + '</div>'
          + '<div class="hint">' + tpl("ดาวใช้อักขระ ★ / ☆ พิมพ์เองได้ · แก้ตัวเลขคะแนนและข้อความได้ตามใจ", "Stars use ★ / ☆ characters you can edit · change the score number and text freely") + '</div>';
      case "faq":
        var faqSnip = '<div class="bk-faq">\n'
          + '  <details open><summary>' + tpl("คำถามที่พบบ่อยข้อที่ 1?", "First frequently asked question?") + '</summary><div>' + tpl("คำตอบแบบละเอียด อธิบายให้ผู้อ่านเข้าใจง่าย", "A clear, detailed answer for your readers.") + '</div></details>\n'
          + '  <details><summary>' + tpl("คำถามที่ 2?", "Second question?") + '</summary><div>' + tpl("คำตอบข้อที่ 2", "Answer to the second question.") + '</div></details>\n'
          + '</div>';
        var faqBtn = function (label, snip) { return '<div style="display:flex;align-items:center;gap:8px;margin-bottom:8px">' +
          '<code style="flex:1;font-size:11.5px;color:#c7c9e0">' + esc(label) + '</code>' +
          '<button class="callout-copy" data-copy="' + esc(snip) + '" style="flex:none;border:1px solid rgba(255,255,255,.16);background:rgba(255,255,255,.06);color:#e9eaf2;border-radius:7px;padding:5px 11px;font-size:12px;cursor:pointer">' + tpl("คัดลอก", "Copy") + '</button></div>'; };
      ...
      case "morefrom": return seg("mode", "โหมด", p.mode || "recent", [["recent", "ล่าสุด"], ["random", "สุ่ม"]])
        ...
        + txt("label", "กรองตามป้ายกำกับ (เว้นว่าง = ทุกป้าย)", p.label || "")
        + num("count", "จำนวนบทความ", p.count || 6, 3, 12)
        + seg("columns", "จำนวนคอลัมน์", String(p.columns || 3), [["2", "2"], ["3", "3"], ["4", "4"]])
        + tog("showImage", "แสดงรูปภาพ", p.showImage !== false)
        + '<div class="note ok">' + svg('<path d="M20 6L9 17l-5-5"/>', 2.5) + '<div>' + tpl("ดึงบทความจาก Feed ของบล็อกอัตโนมัติ · โหมดสุ่มจะสุ่มจากบทความล่าสุด 50 รายการ · ใส่ชื่อป้ายกำกับให้ตรงเพื่อกรองเฉพาะหมวด", "Auto-fetches from the blog Feed · random mode shuffles the latest 50 posts · enter an exact label name to filter to one category") + '</div></div>';
      case "slider":
        var slSnip = '<div class="bk-slider">\n'
          + '  <figure><img src="' + tpl("ลิงก์รูปที่-1.jpg", "IMAGE-URL-1.jpg") + '" alt="' + tpl("คำอธิบายรูป 1", "Image 1 alt") + '"><figcaption>' + tpl("คำบรรยายใต้ภาพที่ 1", "Caption for image 1") + '</figcaption></figure>\n'
          + '  <figure><img src="' + tpl("ลิงก์รูปที่-2.jpg", "IMAGE-URL-2.jpg") + '" alt="' + tpl("คำอธิบายรูป 2", "Image 2 alt") + '"><figcaption>' + tpl("คำบรรยายใต้ภาพที่ 2", "Caption for image 2") + '</figcaption></figure>\n'
          + '  <figure><img src="' + tpl("ลิงก์รูปที่-3.jpg", "IMAGE-URL-3.jpg") + '" alt="' + tpl("คำอธิบายรูป 3", "Image 3 alt") + '"><figcaption>' + tpl("คำบรรยายใต้ภาพที่ 3", "Caption for image 3") + '</figcaption></figure>\n'
          + '</div>';
        var slBtn = function (label, snip) { return '<div style="display:flex;align-items:center;gap:8px;margin-bottom:8px">' +
          '<code style="flex:1;font-size:11.5px;color:#c7c9e0">' + esc(label) + '</code>' +
          '<button class="callout-copy" data-copy="' + esc(snip) + '" style="flex:none;border:1px solid rgba(255,255,255,.16);background:rgba(255,255,255,.06);color:#e9eaf2;border-radius:7px;padding:5px 11px;font-size:12px;cursor:pointer">' + tpl("คัดลอก", "Copy") + '</button></div>'; };
        return '<div class="note info">' + svg('<circle cx="12" cy="12" r="9"/><path d="M12 8h.01M11 12h1v4h1"/>', 2) + '<div>' + tpl('สไลด์รูปเลื่อนได้พร้อมคำบรรยาย · กด “คัดลอก” วางในโพสต์ Blogger → มุมมอง HTML แล้วแก้ลิงก์รูป · เพิ่ม <code>&lt;figure&gt;</code> ได้ไม่จำกัด', 'A swipeable image slider with captions · click “Copy”, paste in Blogger → HTML view and set your image URLs · add as many <code>&lt;figure&gt;</code> as you like') + '</div></div>'
          + '<div class="field"><label>' + tpl("คัดลอกโค้ดสไลด์", "Copy the slider snippet") + '</label>' + slBtn(tpl("สไลด์รูป 3 ภาพ", "3-image slider"), slSnip) + '</div>'
          + '<div class="hint">' + tpl("รองรับปัดนิ้ว (มือถือ) + ลูกศร + จุดบอกตำแหน่ง · ใช้ลิงก์รูปจาก Blogger ได้เลย", "Supports swipe (mobile) + arrows + dot indicators · use your Blogger image URLs") + '</div>';
      case "sharebar": return seg("side", "ตำแหน่งแถบ", p.side || "left", [["left", "◧ ซ้าย"], ["right", "ขวา ◨"]])
        + tog("facebook", "Facebook", p.facebook !== false)
        + tog("twitter", "X (Twitter)", p.twitter !== false)
      ...
      case "lightbox": return '<div class="note ok">' + svg('<path d="M20 6L9 17l-5-5"/>', 2.5) + '<div>' + tpl("คลิกรูปในเนื้อหาบทความเพื่อขยายเต็มจอ · เลื่อนดูรูปถัดไป/ก่อนหน้า ปิดด้วย ✕, แตะพื้นหลัง หรือปุ่ม Esc", "Click any image in the post content to view it fullscreen · navigate prev/next, close with ✕, backdrop tap, or Esc") + '</div></div>'
  ...
  function bindFields(b, c) {
    ...
    $$("[data-catv]", c).forEach(function (inp) {
      inp.addEventListener("input", function () {
        var i = +inp.dataset.catv, arr = catsOf(b.props), t = arr[i].linkType || "label";
        if (t === "page") arr[i].pageSlug = inp.value; else if (t === "label") arr[i].labelName = inp.value; else arr[i].url = inp.value;
        b.props.categories = arr; renderCanvas(); save();
        var chip = inp.parentElement && inp.parentElement.querySelector(".url-chip");
        if (chip) { var u = catUrlFor(arr[i]); chip.textContent = u || tpl("ยังไม่มีลิงก์ · หมวดนี้จะถูกข้าม", "No link yet, this row is skipped"); chip.classList.toggle("empty", !u); }
      });
    });
    $$("[data-catdel]", c).forEach(function (btn) { btn.addEventListener("click", function () { var arr = catsOf(b.props); arr.splice(+btn.dataset.catdel, 1); b.props.categories = arr; commit(); renderProps(); }); });
    var catadd = c.querySelector("[data-catadd]");
    if (catadd) catadd.addEventListener("click", function () {
      var arr = catsOf(b.props);
      arr.push({ id: uid(), label: tpl("หมวดหมู่ใหม่", "New category"), linkType: "label", labelName: "", icon: "", count: "" });
      b.props.categories = arr; commit(); renderProps();
    });
  ...
  function affiliateStatic(p) {
    ...
    var css = "<style>"
      ...
      // ── fullscreen viewer ──
      + ".bxb-glb{position:fixed;inset:0;z-index:99999;background:rgba(8,10,16,.95);display:none;align-items:center;justify-content:center;padding:22px}"
      + ".bxb-glb.on{display:flex}"
      + "body.bxb-glb-open{overflow:hidden}"
      + ".bxb-glb-img{max-width:100%;max-height:86vh;width:auto;height:auto;object-fit:contain;border-radius:10px;display:block;margin:0}"
      + ".bxb-glb-x{position:absolute;top:14px;right:14px;width:44px;height:44px;border-radius:50%;border:0;background:rgba(255,255,255,.16);color:#fff;font-size:22px;line-height:1;cursor:pointer;display:grid;place-items:center;transition:background .15s;-webkit-tap-highlight-color:transparent}"
      + ".bxb-glb-x:hover{background:rgba(255,255,255,.3)}"
      + ".bxb-glb-nav{position:absolute;top:50%;transform:translateY(-50%);width:46px;height:46px;border-radius:50%;border:0;background:rgba(255,255,255,.14);color:#fff;font-size:26px;line-height:1;cursor:pointer;display:grid;place-items:center;transition:background .15s}"
      + ".bxb-glb-nav:hover{background:rgba(255,255,255,.28)}"
      + ".bxb-glb-prev{left:14px}.bxb-glb-next{right:14px}"
      + ".bxb-glb-c{position:absolute;bottom:18px;left:50%;transform:translateX(-50%);color:#fff;font-size:13px;font-weight:600;background:rgba(255,255,255,.14);padding:5px 14px;border-radius:99px}"
      + "@media(max-width:600px){.bxb-glb-nav{width:38px;height:38px;font-size:21px}.bxb-glb-prev{left:6px}.bxb-glb-next{right:6px}}"
    ...
    var sc = "<script>/*<![CDATA[*/(function(){"
      ...
      + "var lb=document.createElement('div');lb.className='bxb-glb';lb.setAttribute('role','dialog');lb.setAttribute('aria-modal','true');"
      + "var im=document.createElement('img');im.className='bxb-glb-img';im.alt='';lb.appendChild(im);"
      + "var x=document.createElement('button');x.type='button';x.className='bxb-glb-x';x.setAttribute('aria-label','Close');x.innerHTML='&#10005;';lb.appendChild(x);"
      + "var pv=document.createElement('button');pv.type='button';pv.className='bxb-glb-nav bxb-glb-prev';pv.setAttribute('aria-label','Previous');pv.innerHTML='&#8249;';lb.appendChild(pv);"
      + "var nx=document.createElement('button');nx.type='button';nx.className='bxb-glb-nav bxb-glb-next';nx.setAttribute('aria-label','Next');nx.innerHTML='&#8250;';lb.appendChild(nx);"
      + "var c=document.createElement('div');c.className='bxb-glb-c';lb.appendChild(c);"
      ...
      + "function doOpen(n){show(n);lb.classList.add('on');document.body.classList.add('bxb-glb-open');open=true;x.focus();}"
      + "function doClose(){lb.classList.remove('on');document.body.classList.remove('bxb-glb-open');open=false;}"
  ...
  function renderDesign() {
    ...
    c.innerHTML =
      ...
      '</div>' +
      contrastNote(d.primary, d.accent) +
  ...
  // Darken a brand colour just far enough that white text on it clears the WCAG target.
  // Only the fills that actually carry text use this; borders, icons and glows keep the
  // colour the user picked, so the brand still reads as theirs.
  function inkFor(hex, target) {
    var t = target || 4.5, c = hexToRgb(hex), out = rgbToHex(c), i = 0;
    while (contrastRatio(out, "#ffffff") < t && i++ < 300) {
      c = { r: c.r * 0.985, g: c.g * 0.985, b: c.b * 0.985 };
      out = rgbToHex(c);
      if (c.r < 0.6 && c.g < 0.6 && c.b < 0.6) break;
    }
    return out;
  }
  // Live readability report for the picked brand colour. The theme already corrects itself,
  // so this explains what it did rather than leaving the user with a warning they cannot act on.
  function contrastNote(primary, accent) {
    var warnIcon = '<path d="M10.3 3.9L1.8 18a2 2 0 001.7 3h17a2 2 0 001.7-3L13.7 3.9a2 2 0 00-3.4 0z"/><path d="M12 9v4M12 17h.01"/>';
    var r = contrastRatio(primary, "#ffffff"), ratio = r.toFixed(2);
    if (r >= 4.5) {
      return '<div class="note ok" style="margin:0 16px 14px">' + svg('<path d="M20 6L9 17l-5-5"/>', 2.5) + '<div>' +
        tpl('ตัวอักษรขาวบนสีหลัก <b>' + ratio + ':1</b> ผ่านมาตรฐาน WCAG AA ✓',
            'White text on the primary colour is <b>' + ratio + ':1</b>, passing WCAG AA ✓') + '</div></div>';
    }
    var ink = inkFor(primary), inkR = contrastRatio(ink, "#ffffff").toFixed(2);
    var accInk = accent ? inkFor(accent) : "";
    return '<div class="note warn" style="margin:0 16px 14px">' + svg(warnIcon, 2) + '<div>' +
      tpl('ตัวอักษรขาวบนสีนี้ได้ <b>' + ratio + ':1</b> ต่ำกว่ามาตรฐาน WCAG AA (4.5:1) อ่านยาก',
          'White text on this colour is <b>' + ratio + ':1</b>, below the WCAG AA minimum of 4.5:1.') +
      '<div style="margin-top:7px;line-height:1.65">' +
      tpl('ธีมจะใช้เฉดเข้มขึ้น <b>' + ink + '</b> (' + inkR + ':1) เฉพาะปุ่มและป้ายที่มีตัวหนังสือทับให้อัตโนมัติ ส่วนเส้นขอบ ไอคอน และเงา ยังเป็นสีที่คุณเลือก',
          'Buttons and badges that carry text will automatically use <b>' + ink + '</b> (' + inkR + ':1). Borders, icons and shadows keep the colour you picked.') +
      '</div>' +
      '<button type="button" class="ctr-fix" data-ctr-fix="' + ink + '"' + (accInk ? ' data-ctr-fix-acc="' + accInk + '"' : '') + '>' +
      tpl('เปลี่ยนสีหลักเป็นเฉดนี้เลย', 'Use that shade as my primary') + '</button>' +
      '</div></div>';
  }
  function bindDesign(c) {
    ...
    var ctrFix = c.querySelector("[data-ctr-fix]");
    if (ctrFix) ctrFix.addEventListener("click", function () {
      S.design.primary = ctrFix.dataset.ctrFix;
      if (ctrFix.dataset.ctrFixAcc) S.design.accent = ctrFix.dataset.ctrFixAcc;
      renderDesign(); renderCanvas(); save();
    });
  ...
  function genXML() {
    ...
    // bar on the review template: that bar is inserted at post-body's firstChild by the
    // affiliate script at runtime, so it pushes in above whatever is already there.
    var shareBlock = S.blocks.find(function (b) { return b.type === "share"; });
    var sharePos = shareBlock ? (shareBlock.props.pos || "bottom") : "";
    var shareTopHtml = shareBlock && sharePos !== "bottom" ? shareCardHtml(shareBlock.props, "top") : "";
    var shareBottomHtml = shareBlock && sharePos !== "top" ? shareCardHtml(shareBlock.props, "bottom") : "";
    var shareJsHtml = shareBlock ? shareScriptHtml(shareBlock.props) : "";
...
":root{--primary:$(keycolor);--accent:$(accentcolor);--primary-ink:" + inkFor(d.primary) + ";--accent-ink:" + inkFor(d.accent) + ";--radius:$(radius);--bg-body:#fff;--bg-surface:#f7f8fc;--bg-surface-2:#f0f1f7;--bg-header:#fff;--text-main:#1e2333;--text-muted:#4a5063;--text-subtle:#6f7689;--border:rgba(0,0,0,.07);--border-med:rgba(0,0,0,.1);--hover-bg:rgba(0,0,0,.05);--nav-shadow:0 0 48px rgba(0,0,0,.25);--drop-shadow:0 8px 24px rgba(0,0,0,.12)}",
tplStyleVars(),
...
".nav-search-pop button[type=submit]{flex:none;border:0;background:var(--primary-ink);color:#fff;font-family:inherit;font-weight:600;font-size:13.5px;padding:9px 15px;border-radius:9px;cursor:pointer}",
...
".nav-drawer-search button{flex:none;border:0;background:var(--primary-ink);color:#fff;font-family:inherit;font-weight:600;font-size:13.5px;padding:9px 14px;border-radius:10px;cursor:pointer}",
...
".blog-pager a:hover{background:var(--primary-ink);border-color:var(--primary-ink);color:#fff;transform:translateY(-2px);box-shadow:0 10px 22px -8px rgba(0,0,0,.35)}",
...
".site-footer{background:var(--footer-bg);color:var(--footer-fg);padding:52px 20px 28px;border-top:3px solid var(--primary,#6366f1)}",
".footer-grid{display:grid;grid-template-columns:1.5fr 1fr;gap:48px;max-width:980px;margin:0 auto;align-items:start}",
".footer-grid--cats{grid-template-columns:1.4fr .8fr 1.3fr;gap:40px}",
"@media(max-width:900px){.footer-grid--cats{grid-template-columns:1fr 1fr;gap:32px}.footer-grid--cats .footer-cats{grid-column:1/-1}}",
"@media(max-width:768px){.footer-grid,.footer-grid--cats{grid-template-columns:1fr;gap:28px}}",
...
".footer-link:hover{color:var(--footer-fg);color:color-mix(in srgb,var(--primary,#6366f1) 60%,var(--footer-fg))}",
...
// The card can now sit inside .post-body, where .post-body a (0,1,1) outranks
// .bxb-share-btn (0,1,0) and would recolour and underline every button.
".bxb-share-card .bxb-share-btn{color:#fff;text-decoration:none}",
...
"@media(prefers-reduced-motion:reduce){.pb-hero::before{animation:none}}",
".pb-hero::after{content:'';position:absolute;left:-40px;bottom:-40px;width:180px;height:180px;border-radius:50%;background:radial-gradient(circle at 35% 35%,color-mix(in srgb,var(--accent) 22%,transparent),transparent 68%)}",
".pb-hero .wrap{position:relative;z-index:1;display:flex;align-items:center;gap:56px;flex-wrap:wrap;max-width:900px}",
".pb-eyebrow{display:inline-block;font-size:11px;color:#fff;background:linear-gradient(120deg,var(--primary-ink),var(--accent-ink));font-weight:700;letter-spacing:.18em;text-transform:uppercase;padding:7px 16px;border-radius:999px;transform:rotate(-2deg);box-shadow:0 6px 16px -6px color-mix(in srgb,var(--primary) 55%,transparent);margin-bottom:20px}",
".pb-avatar-ring{width:180px;height:180px;border-radius:50%;padding:5px;background:conic-gradient(from 210deg,var(--primary),var(--accent),var(--primary));flex:none;box-shadow:0 18px 44px -16px color-mix(in srgb,var(--primary) 45%,transparent),0 2px 6px rgba(0,0,0,.06)}",
".pb-avatar-inner{width:100%;height:100%;border-radius:50%;background:var(--bg-base);display:flex;align-items:center;justify-content:center;box-shadow:inset 0 0 0 3px var(--bg-base)}",
".pb-grid{display:grid;gap:24px}",
".pb-card{position:relative;border-radius:var(--radius);overflow:hidden;background:var(--bg-surface);border:1px solid color-mix(in srgb,var(--border) 60%,transparent);box-shadow:0 1px 2px rgba(20,16,8,.04),0 6px 18px -10px rgba(20,16,8,.08);transition:transform .3s cubic-bezier(.22,1,.36,1),box-shadow .3s cubic-bezier(.22,1,.36,1),border-color .3s}",
".pb-card::before{content:'';position:absolute;top:0;left:0;right:0;height:4px;background:linear-gradient(90deg,var(--primary),var(--accent));z-index:1}",
".pb-card:hover{transform:translateY(-6px) rotate(-.4deg);border-color:color-mix(in srgb,var(--primary) 25%,var(--border));box-shadow:0 2px 4px rgba(20,16,8,.04),0 24px 48px -18px rgba(20,16,8,.16)}",
".pb-card-img{height:190px;overflow:hidden;background:var(--hover-bg)}",
".pb-card-img img{width:100%;height:100%;object-fit:cover;display:block;transition:transform .8s cubic-bezier(.22,1,.36,1)}",
".pb-card:hover .pb-card-img img{transform:scale(1.06)}",
".pb-card-body{padding:20px 22px 24px}",
".pb-card-date{display:inline-block;font-size:10.5px;color:var(--primary);background:color-mix(in srgb,var(--primary) 10%,transparent);letter-spacing:.12em;text-transform:uppercase;font-weight:700;padding:4px 10px;border-radius:999px;margin-bottom:10px}",
".pb-card-title{font-size:18px;font-weight:700;line-height:1.38;margin:0 0 8px;font-family:var(--font);letter-spacing:-.01em}",
".pb-card-title a{color:var(--text-main);text-decoration:none;background-image:linear-gradient(color-mix(in srgb,var(--primary) 40%,transparent),color-mix(in srgb,var(--primary) 40%,transparent));background-size:0 1px;background-position:0 100%;background-repeat:no-repeat;transition:background-size .35s,color .2s}",
".pb-card-title a:hover{color:var(--primary);background-size:100% 1px}",
".pb-card-excerpt{font-size:13.5px;color:var(--text-muted);line-height:1.65;overflow:hidden;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;margin:0 0 14px}",
".pb-card-read{display:inline-flex;align-items:center;gap:6px;font-size:12.5px;color:var(--primary);font-weight:700;letter-spacing:.02em;text-decoration:none;transition:gap .25s}",
".pb-card-read:hover{gap:10px}",
...
".pb-about{position:relative;overflow:hidden;padding:72px 20px;background:radial-gradient(circle,color-mix(in srgb,var(--accent) 10%,transparent) 1.3px,transparent 1.7px) 0 0/26px 26px,linear-gradient(color-mix(in srgb,var(--primary) 4%,var(--bg-surface)),var(--bg-surface));flex:1 0 auto;display:flex;flex-direction:column;justify-content:center;border-top:1px solid color-mix(in srgb,var(--border) 70%,transparent)}",
".pb-about::after{content:'';position:absolute;top:-50px;right:-50px;width:190px;height:190px;border-radius:50%;border:2px dashed color-mix(in srgb,var(--accent) 32%,transparent)}",
".pb-about>.wrap{width:100%}",
".pb-about .wrap{display:flex;gap:28px;align-items:flex-start;flex-wrap:wrap;max-width:800px}",
".pb-about-avatar{width:110px;height:110px;border-radius:50%;padding:4px;background:conic-gradient(from 210deg,var(--primary),var(--accent),var(--primary));flex:none;box-shadow:0 12px 28px -12px color-mix(in srgb,var(--primary) 40%,transparent)}",
...
".pb-about-eyebrow{display:inline-block;font-size:10.5px;color:#fff;background:linear-gradient(120deg,var(--primary-ink),var(--accent-ink));font-weight:700;letter-spacing:.16em;text-transform:uppercase;padding:6px 14px;border-radius:999px;transform:rotate(-1.5deg);margin-bottom:12px}",
".pb-about-name{font-size:26px;font-weight:800;color:var(--text-main);margin:0 0 10px;line-height:1.2;font-family:var(--font);letter-spacing:-.015em}",
...
".tech-hero-btn{display:inline-block;background:var(--primary-ink);color:#fff;font-weight:700;padding:12px 28px;border-radius:var(--radius);text-decoration:none;font-size:15px}",
...
".tech-card-tag{position:absolute;top:10px;left:10px;font-size:11px;font-weight:700;letter-spacing:.04em;padding:3px 9px;border-radius:20px;background:var(--primary-ink);color:#fff}",
...
".mag-feat-cat{font-size:10px;font-weight:700;letter-spacing:.09em;text-transform:uppercase;background:var(--primary-ink);color:#fff;padding:3px 10px;border-radius:3px;display:inline-block;margin-bottom:10px}",
...
".mag-side-cat{font-size:10px;font-weight:700;letter-spacing:.07em;text-transform:uppercase;background:var(--primary-ink);color:#fff;padding:2px 9px;border-radius:3px;display:inline-block;margin-bottom:6px}",
...
".mag-news-cat{font-size:10px;font-weight:700;letter-spacing:.06em;text-transform:uppercase;color:#fff;background:var(--primary-ink);padding:2px 8px;border-radius:3px;white-space:nowrap;flex:none}",
...
".corp-hero{padding:88px 20px;background:linear-gradient(135deg,var(--primary-ink),var(--accent-ink));color:#fff;position:relative;overflow:hidden}",
...
".corp-cta{padding:88px 20px;background:radial-gradient(circle at 8% 12%,transparent 30%,rgba(255,255,255,.06) 30.4%,rgba(255,255,255,.06) 33%,transparent 33.4%),radial-gradient(60% 90% at 92% 90%,rgba(255,255,255,.14),transparent 60%),linear-gradient(135deg,var(--primary-ink),var(--accent-ink));text-align:center;color:#fff}",
...
".edu-hero{position:relative;overflow:hidden;padding:88px 20px;background:radial-gradient(circle at 12% 18%,rgba(255,255,255,.17),transparent 26%),radial-gradient(circle at 88% 84%,rgba(255,255,255,.13),transparent 24%),linear-gradient(135deg,var(--primary-ink),var(--accent-ink));color:#fff}",
...
".edu-card-enroll{font-size:13px;font-weight:700;color:#fff;background:var(--primary-ink);padding:7px 16px;border-radius:calc(var(--radius)*0.7);text-decoration:none;display:inline-block}",
".edu-cta{position:relative;padding:88px 20px;background:radial-gradient(circle at 15% 22%,rgba(255,255,255,.15),transparent 25%),radial-gradient(circle at 85% 78%,rgba(255,255,255,.12),transparent 22%),linear-gradient(135deg,var(--primary-ink),var(--accent-ink));text-align:center;color:#fff}",
...
".rv-hero{position:relative;overflow:hidden;padding:82px 20px;background:linear-gradient(135deg,var(--primary-ink),var(--accent-ink));color:#fff}",
...
".rv-cat-search{display:flex;align-items:center;gap:8px;margin:2px 4px 8px;padding:2px 6px 2px 12px;border:1px solid var(--border-med);border-radius:11px;background:var(--bg-surface)}",
".rv-cat-search svg{width:16px;height:16px;flex:none;color:var(--text-subtle)}",
".rv-cat-search input{flex:1;min-width:0;border:0;background:none;padding:9px 0;font-size:14px;font-family:inherit;color:var(--text-main);outline:none}",
".rv-cat-none{margin:6px 4px 4px;padding:14px;text-align:center;font-size:13px;color:var(--text-muted)}",
".rv-cat-scrim,.rv-cat-sheet{display:none}",
"@media(max-width:768px){",
".rv-catli{display:none}",
"body.rv-cat-lock{overflow:hidden}",
".rv-cat-scrim{display:block;position:fixed;inset:0;z-index:210;background:rgba(0,0,0,.45);backdrop-filter:blur(2px);opacity:0;pointer-events:none;transition:opacity .25s}",
".rv-cat-scrim.open{opacity:1;pointer-events:auto}",
".rv-cat-sheet{display:flex;flex-direction:column;position:fixed;left:0;right:0;bottom:0;z-index:220;background:var(--bg-header);border-radius:20px 20px 0 0;box-shadow:0 -10px 40px rgba(0,0,0,.22);padding:0 10px calc(14px + env(safe-area-inset-bottom,0px));max-height:78vh;transform:translateY(102%);visibility:hidden;transition:transform .3s cubic-bezier(.22,1,.36,1),visibility .3s}",
".rv-cat-sheet.open{transform:translateY(0);visibility:visible}",
".rv-cat-sheet-head{position:relative;flex:none;display:flex;align-items:center;padding:16px 6px 12px;border-bottom:1px solid var(--border)}",
".rv-cat-sheet-head::before{content:'';position:absolute;top:7px;left:50%;width:40px;height:4px;margin-left:-20px;border-radius:99px;background:var(--border-med)}",
".rv-cat-sheet-head h3{margin:0;font-size:16px;font-weight:700;color:var(--text-main)}",
".rv-cat-x{margin-left:auto;width:34px;height:34px;flex:none;display:grid;place-items:center;border-radius:50%;border:1px solid var(--border-med);background:transparent;color:var(--text-main);font-size:15px;cursor:pointer;-webkit-tap-highlight-color:transparent}",
".rv-cat-x:active{background:var(--hover-bg)}",
".rv-cat-sheet .rv-cat-search{margin:12px 4px 4px}",
".rv-cat-sheet .rv-cat-list{overflow-y:auto;-webkit-overflow-scrolling:touch;padding:6px 0 4px;grid-template-columns:1fr}",
".rv-cat-sheet .rv-cat-list a{padding:13px 10px;font-size:15px}",
"}",
".footer-cats-head{display:flex;align-items:center;gap:9px;font-size:13px;font-weight:700;letter-spacing:.03em;color:var(--footer-fg);margin:0 0 14px}",
".footer-cats-head::before{content:'';flex:none;width:4px;height:15px;border-radius:99px;background:linear-gradient(180deg,var(--primary),var(--accent))}",
".footer-cats-wrap{display:flex;flex-wrap:wrap;gap:8px}",
".footer-cat{display:inline-flex;align-items:center;gap:6px;font-size:12.5px;color:var(--footer-muted);background:var(--footer-chip);border:1px solid transparent;border-radius:99px;padding:6px 13px;text-decoration:none;transition:color .2s,background .2s,border-color .2s,transform .18s}",
".footer-cat:hover{color:var(--footer-fg);background:transparent;border-color:color-mix(in srgb,var(--primary) 55%,transparent);transform:translateY(-2px)}",
".footer-cat-n{opacity:.62;font-size:11px;font-weight:700}",
...
".rv-card-best{position:absolute;top:17px;left:-38px;width:150px;z-index:3;transform:rotate(-45deg);text-align:center;font-size:10px;font-weight:800;letter-spacing:.07em;line-height:1.35;text-transform:uppercase;padding:6px 0;color:#fff;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;background:linear-gradient(135deg,var(--primary)40,var(--accent)26);-webkit-backdrop-filter:saturate(1.25) brightness(.86);backdrop-filter:saturate(1.25) brightness(.86);box-shadow:inset 0 1px 0 rgba(255,255,255,.4),inset 0 -1px 0 rgba(255,255,255,.22);text-shadow:0 1px 3px rgba(0,0,0,.62),0 0 9px rgba(0,0,0,.42);transition:background .25s}",
".rv-card:hover .rv-card-best{background:linear-gradient(135deg,var(--primary)a6,var(--accent)80)}",
"@supports not ((backdrop-filter:blur(2px)) or (-webkit-backdrop-filter:blur(2px))){.rv-card-best{background:linear-gradient(135deg,var(--primary)8c,var(--accent)73)}}",
...
".rv-card-deal{display:inline-flex;align-items:center;gap:6px;font-size:13px;font-weight:700;color:#fff;background:linear-gradient(135deg,var(--primary-ink),var(--accent-ink));padding:9px 19px;border-radius:99px;text-decoration:none;white-space:nowrap;box-shadow:0 3px 10px var(--primary)3d;transition:transform .18s,box-shadow .18s,filter .18s}",
...
".sb-hero-btn{display:inline-block;background:var(--primary-ink);color:#fff;font-weight:700;padding:12px 26px;border-radius:var(--radius);text-decoration:none;font-size:15px}",
...
/* ═══════════ PREMIUM GRAPHIC LAYER · entrance animations + micro-interactions (additive overrides) ═══════════ */
"@keyframes bxbFadeUp{from{opacity:0;transform:translateY(22px)}to{opacity:1;transform:none}}",
"@keyframes bxbFadeIn{from{opacity:0}to{opacity:1}}",
"@keyframes bxbPop{0%{transform:scale(1)}45%{transform:scale(1.18)}100%{transform:scale(1)}}",
"@keyframes bxbSheen{from{transform:translateX(-130%) skewX(-18deg)}to{transform:translateX(240%) skewX(-18deg)}}",
"@keyframes bxbFloat{0%,100%{transform:translateY(0)}50%{transform:translateY(-7px)}}",
/* hero entrances (load-time, once) */
".pb-hero .wrap,.tb-hero-content,.tech-hero .wrap,.corp-hero .wrap,.edu-hero .wrap,.rv-hero .wrap,.sb-hero .wrap,.mag-feat .wrap,.hero .wrap{animation:bxbFadeUp .7s cubic-bezier(.22,1,.36,1) both}",
/* ── desktop nav: sliding underline + animated dropdown ── */
".site-nav li a{position:relative}",
".site-nav>ul>li>a::after,.site-nav>ul>li>.rv-catbtn::after{content:'';position:absolute;left:12px;right:100%;bottom:4px;height:2px;border-radius:2px;background:linear-gradient(90deg,var(--primary),var(--accent));transition:right .28s cubic-bezier(.22,1,.36,1)}",
".site-nav>ul>li>a:hover::after,.site-nav>ul>li>a[aria-current='page']::after,.site-nav>ul>li>.rv-catbtn:hover::after,.site-nav>ul>li>.rv-catbtn[aria-expanded='true']::after{right:12px}",
"@media(min-width:769px){.site-nav .dropdown{display:block;opacity:0;visibility:hidden;transform:translateY(8px) scale(.98);transform-origin:top left;transition:opacity .22s,transform .22s cubic-bezier(.22,1,.36,1),visibility .22s;pointer-events:none}",
".site-nav li:hover>.dropdown,.site-nav li:focus-within>.dropdown{opacity:1;visibility:visible;transform:none;pointer-events:auto}",
".site-nav .dropdown li a{transition:background .15s,color .15s,padding-left .2s}",
".site-nav .dropdown li a:hover{padding-left:22px}}",
/* ── mobile drawer: staggered slide-in ── */
"@media(max-width:768px){",
".site-nav>ul>li{opacity:0;transform:translateX(18px)}",
".site-nav.nav-left>ul>li{transform:translateX(-18px)}",
".nav-toggle-cb:checked~.site-bar .site-nav>ul>li{opacity:1;transform:none;transition:opacity .34s ease,transform .34s cubic-bezier(.22,1,.36,1)}",
".nav-toggle-cb:checked~.site-bar .site-nav>ul>li:nth-child(1){transition-delay:.05s}",
".nav-toggle-cb:checked~.site-bar .site-nav>ul>li:nth-child(2){transition-delay:.09s}",
".nav-toggle-cb:checked~.site-bar .site-nav>ul>li:nth-child(3){transition-delay:.13s}",
".nav-toggle-cb:checked~.site-bar .site-nav>ul>li:nth-child(4){transition-delay:.17s}",
".nav-toggle-cb:checked~.site-bar .site-nav>ul>li:nth-child(5){transition-delay:.21s}",
".nav-toggle-cb:checked~.site-bar .site-nav>ul>li:nth-child(6){transition-delay:.25s}",
".nav-toggle-cb:checked~.site-bar .site-nav>ul>li:nth-child(n+7){transition-delay:.29s}",
".nav-toggle-cb:checked~.site-bar .nav-scrim{animation:bxbFadeIn .28s both}",
".site-nav li a:active{transform:scale(.97);transition:transform .1s}",
"}",
/* ── sidebar-blog · warm editorial ── */
".sb-hero{position:relative;overflow:hidden}",
".sb-hero::after{content:'';position:absolute;top:-60px;right:-60px;width:230px;height:230px;border-radius:50%;border:2px dashed color-mix(in srgb,var(--primary) 30%,transparent);pointer-events:none}",
".sb-card{border:1px solid color-mix(in srgb,var(--border) 60%,transparent);transition:transform .3s cubic-bezier(.22,1,.36,1),box-shadow .3s,border-color .3s}",
".sb-card:hover{transform:translateY(-5px);border-color:color-mix(in srgb,var(--primary) 30%,var(--border));box-shadow:0 20px 44px -18px rgba(20,16,8,.18)}",
".sb-card-read{display:inline-flex;align-items:center;gap:6px;transition:gap .25s}",
".sb-card-read:hover{gap:10px;text-decoration:none}",
".sb-h2{position:relative;display:inline-block;padding-bottom:10px}",
".sb-h2::after{content:'';position:absolute;left:0;bottom:0;width:44px;height:3px;border-radius:2px;background:linear-gradient(90deg,var(--primary),var(--accent))}",
/* ── review · trust & shine ── */
".rv-hero{position:relative;overflow:hidden}",
".rv-hero::before{content:'';position:absolute;inset:0;background:radial-gradient(circle,color-mix(in srgb,var(--accent) 10%,transparent) 1.4px,transparent 1.8px) 0 0/26px 26px;pointer-events:none}",
".rv-card{position:relative;transition:transform .3s cubic-bezier(.22,1,.36,1),box-shadow .3s}",
".rv-card::after{content:'';position:absolute;top:0;left:0;right:0;height:4px;background:linear-gradient(90deg,var(--primary),var(--accent));opacity:0;transition:opacity .3s}",
".rv-card:hover{transform:translateY(-6px);box-shadow:0 22px 48px -18px rgba(20,16,8,.2)}",
".rv-card:hover::after{opacity:1}",
".rv-about-badge{position:relative;overflow:hidden}",
".rv-about-badge::after{content:'';position:absolute;inset:0;background:linear-gradient(105deg,transparent 38%,rgba(255,255,255,.5) 50%,transparent 62%);transform:translateX(-130%) skewX(-18deg);animation:bxbSheen 3.4s ease 1.2s infinite}",
/* ── magazine · bold press ── */
".mag-feat-cat,.mag-side-cat,.mag-news-cat{transform:skewX(-6deg);box-shadow:0 3px 10px color-mix(in srgb,var(--primary) 40%,transparent)}",
".mag-feat-main,.mag-side-item{transition:transform .3s cubic-bezier(.22,1,.36,1),box-shadow .3s}",
".mag-feat-main:hover,.mag-side-item:hover{transform:translateY(-4px);box-shadow:0 22px 48px -16px rgba(0,0,0,.3)}",
".mag-news-row{transition:background .2s,padding-left .25s cubic-bezier(.22,1,.36,1)}",
".mag-news-row:hover{background:color-mix(in srgb,var(--primary) 4%,transparent);padding-left:10px}",
".mag-card{transition:transform .3s cubic-bezier(.22,1,.36,1),box-shadow .3s,border-color .3s}",
".mag-card:hover{transform:translateY(-5px);border-color:color-mix(in srgb,var(--primary) 30%,var(--border));box-shadow:0 18px 40px -16px rgba(0,0,0,.16)}",
/* ── travel · wanderlust ── */
".tb-card-loc{animation:bxbFloat 4.5s ease-in-out infinite}",
".tb-card{transition:transform .35s cubic-bezier(.22,1,.36,1),box-shadow .35s}",
".tb-card:hover{transform:translateY(-7px) rotate(.4deg);box-shadow:0 24px 52px -18px rgba(0,0,0,.2)}",
".tb-card-btn{transition:gap .25s}",
".tb-card-btn:hover{gap:9px}",
".tb-hero-btn{position:relative;overflow:hidden}",
".tb-hero-btn::after{content:'';position:absolute;inset:0;background:linear-gradient(105deg,transparent 38%,rgba(255,255,255,.55) 50%,transparent 62%);transform:translateX(-130%) skewX(-18deg);animation:bxbSheen 4s ease 1.6s infinite}",
/* ── course · playful learning ── */
".edu-card{transition:transform .3s cubic-bezier(.22,1,.36,1),box-shadow .3s,border-color .3s}",
".edu-card:hover{transform:translateY(-6px) rotate(-.5deg);box-shadow:0 22px 46px -18px rgba(20,16,8,.18)}",
".edu-hero-chip{transition:transform .25s cubic-bezier(.34,1.56,.64,1),background .2s}",
".edu-hero-chip:hover{transform:translateY(-3px) scale(1.04)}",
".edu-h2{position:relative;display:inline-block;padding-bottom:10px}",
".edu-h2::after{content:'';position:absolute;left:0;bottom:0;width:52px;height:4px;border-radius:3px;background:linear-gradient(90deg,var(--primary),var(--accent))}",
/* ── company · corporate polish ── */
".corp-hero-btn,.corp-cta-btn{position:relative;overflow:hidden;transition:transform .25s cubic-bezier(.22,1,.36,1),box-shadow .25s}",
".corp-hero-btn:hover,.corp-cta-btn:hover{transform:translateY(-3px);box-shadow:0 16px 34px -10px rgba(0,0,0,.3)}",
".corp-hero-btn::after,.corp-cta-btn::after{content:'';position:absolute;inset:0;background:linear-gradient(105deg,transparent 38%,rgba(255,255,255,.6) 50%,transparent 62%);transform:translateX(-130%) skewX(-18deg)}",
".corp-hero-btn:hover::after,.corp-cta-btn:hover::after{animation:bxbSheen .8s ease}",
".corp-hero-btn-out{transition:background .2s,border-color .2s,transform .25s}",
".corp-hero-btn-out:hover{background:rgba(255,255,255,.12);border-color:rgba(255,255,255,.75);transform:translateY(-3px)}",
".corp-stat-num{background:linear-gradient(120deg,var(--primary),var(--accent));-webkit-background-clip:text;background-clip:text;-webkit-text-fill-color:transparent}",
".corp-visual-block{transition:transform .35s cubic-bezier(.22,1,.36,1)}",
".corp-visual:hover .corp-visual-block:nth-child(odd){transform:translateY(-6px)}",
".corp-visual:hover .corp-visual-block:nth-child(even){transform:translateY(6px)}",
/* ── tech · neon terminal ── */
".tech-card{position:relative;transition:transform .3s cubic-bezier(.22,1,.36,1),border-color .25s,box-shadow .3s}",
".tech-card:hover{transform:translateY(-5px)}",
".tech-list-row{transition:background .2s,padding-left .25s cubic-bezier(.22,1,.36,1)}",
".tech-list-row:hover{background:color-mix(in srgb,var(--primary) 5%,transparent);padding-left:10px}",
".tech-tag{transition:transform .2s cubic-bezier(.34,1.56,.64,1)}",
".tech-tag:hover{transform:translateY(-2px)}",
".tech-hero-btn{transition:transform .22s,box-shadow .22s}",
".tech-hero-btn:hover{transform:translateY(-3px);box-shadow:0 0 0 1px var(--primary),0 14px 34px color-mix(in srgb,var(--primary) 45%,transparent)}",
/* reduced motion: kill all decorative animation */
"@media(prefers-reduced-motion:reduce){.pb-hero .wrap,.tb-hero-content,.tech-hero .wrap,.corp-hero .wrap,.edu-hero .wrap,.rv-hero .wrap,.sb-hero .wrap,.mag-feat .wrap,.hero .wrap,.rv-about-badge::after,.tb-hero-btn::after,.tb-card-loc,.corp-hero-btn::after,.corp-cta-btn::after{animation:none!important}.site-nav>ul>li{opacity:1!important;transform:none!important;transition:none!important}}",

/* ═══════════ PREMIUM LAYER 2 · widgets, footer, post page, utilities ═══════════ */
/* global polish */
...
"body::-webkit-scrollbar-track{background:var(--bg-surface)}",
"body::-webkit-scrollbar-thumb{background:linear-gradient(var(--primary),var(--accent));border-radius:6px;border:3px solid var(--bg-surface)}",
/* sidebar widgets */
".bxb-sidebar-col .widget{border:1px solid color-mix(in srgb,var(--border) 65%,transparent);transition:box-shadow .3s,border-color .3s,transform .3s cubic-bezier(.22,1,.36,1)}",
".bxb-sidebar-col .widget:hover{border-color:color-mix(in srgb,var(--primary) 22%,var(--border));box-shadow:0 12px 30px -14px rgba(20,16,8,.14);transform:translateY(-2px)}",
".widget-title{border-image:linear-gradient(90deg,var(--primary),var(--accent),transparent 82%) 1}",
".widget li{list-style:none}",
".widget ul{padding-left:0;margin:0}",
".widget li a{display:inline-block;text-decoration:none;color:var(--text-main);padding:4px 0;transition:padding-left .22s cubic-bezier(.22,1,.36,1),color .18s}",
".widget li a:hover{padding-left:8px;color:var(--primary)}",
/* footer */
".site-footer{border-top:3px solid transparent;border-image:linear-gradient(90deg,var(--primary),var(--accent),var(--primary)) 1;position:relative}",
".footer-link{transition:color .2s,padding-left .22s cubic-bezier(.22,1,.36,1)}",
".footer-link:hover{padding-left:6px}",
".footer-social-icon{transition:background .2s,transform .22s cubic-bezier(.34,1.56,.64,1),box-shadow .22s}",
".footer-social-icon:hover{transform:translateY(-4px) scale(1.06);box-shadow:0 10px 20px -8px rgba(0,0,0,.35)}",
/* post page */
".post-title{animation:bxbFadeUp .6s cubic-bezier(.22,1,.36,1) both}",
".post-cat{padding:3px 10px;border-radius:999px;background:color-mix(in srgb,var(--primary) 9%,transparent);transition:background .2s,color .2s}",
".post-cat:hover{background:var(--primary-ink);color:#fff;text-decoration:none}",
".post-body h2{background:linear-gradient(90deg,var(--primary),var(--accent)) left bottom/46px 3px no-repeat;padding-bottom:10px}",
".post-body blockquote{position:relative;padding-left:52px}",
".post-body blockquote::before{content:'\\201C';position:absolute;left:14px;top:2px;font-size:44px;line-height:1;color:var(--primary);opacity:.55;font-family:Georgia,serif}",
".post-body table{width:100%;border-collapse:separate;border-spacing:0;border:1px solid var(--border);border-radius:var(--radius);overflow:hidden;margin:20px 0;font-size:14.5px}",
".post-body th{background:color-mix(in srgb,var(--primary) 8%,var(--bg-surface));font-weight:700;text-align:left;padding:11px 14px}",
".post-body td{padding:10px 14px;border-top:1px solid var(--border)}",
".post-body tr:nth-child(even) td{background:color-mix(in srgb,var(--primary) 3%,transparent)}",
".post-body img{box-shadow:0 8px 28px -12px rgba(20,16,8,.18)}",
/* TOC */
"#bxbToc{border-image:linear-gradient(var(--primary),var(--accent)) 1;transition:box-shadow .3s}",
"#bxbToc:hover{box-shadow:0 8px 24px -12px rgba(20,16,8,.12)}",
"#bxbTocList a{display:inline-block;transition:padding-left .2s cubic-bezier(.22,1,.36,1),color .15s}",
"#bxbTocList a:hover{padding-left:6px}",
/* related posts */
"#bxbRelGrid a{transition:transform .28s cubic-bezier(.22,1,.36,1),box-shadow .28s}",
"#bxbRelGrid a:hover{transform:translateY(-4px);box-shadow:0 16px 34px -16px rgba(20,16,8,.18)}",
...
"@media(prefers-reduced-motion:reduce){.pb-avatar-ring,.pb-about-avatar,.rv-hero::before,.rv-about-avatar,.sb-hero::before,.sb-hero::after,.tech-hero::before{animation:none!important}}"
  ...
  function renderBlockStatic(b) {
    ...
    switch (b.type) {
      case "header":
        ...
        // Category dropdown sits second in the bar (right after Home), the spot readers expect it.
        var hCats = reviewCats();
        if (hCats.length) {
          var hcCols = catCols(hCats.length);
          var hCatLi = "<li class='rv-catli'>" +
            "<button type='button' class='rv-catbtn' id='rvCatBtn' aria-haspopup='true' aria-expanded='false' aria-controls='rvCatDrop'>" +
              "<svg class='nav-ic' viewBox='0 0 24 24' fill='none' stroke='currentColor' stroke-width='1.9' stroke-linecap='round' stroke-linejoin='round' aria-hidden='true'><rect x='3' y='3' width='7.5' height='7.5' rx='2'/><rect x='13.5' y='3' width='7.5' height='7.5' rx='2'/><rect x='3' y='13.5' width='7.5' height='7.5' rx='2'/><rect x='13.5' y='13.5' width='7.5' height='7.5' rx='2'/></svg>" +
              "<span>" + esc(catMenuLabel()) + "</span>" +
              "<svg class='rv-cat-caret' viewBox='0 0 24 24' aria-hidden='true'><path d='M7 10l5 5 5-5z' fill='currentColor'/></svg>" +
            "</button>" +
            "<div class='rv-cat-drop' id='rvCatDrop' data-cols='" + hcCols + "'>" +
              catSearchHtml(hCats.length, "rvCatFilterD") +
              "<ul class='rv-cat-list' role='menu'>" + catItemsHtml(hCats) + "</ul>" +
              "<p class='rv-cat-none' hidden='hidden'>" + tpl("ไม่พบหมวดหมู่ที่ค้นหา", "No matching category") + "</p>" +
            "</div></li>";
          hMenu.splice(hMenu.length > 1 ? 1 : hMenu.length, 0, hCatLi);
        }
        hMenu = hMenu.join("");
      ...
      case "hero":
          ...
          return "<section class='pb-hero'>" +
            "<div class='wrap pb-hero-inner'>" +
              "<div class='pb-hero-text'>" +
                ...
                "<a href='#main' style='display:inline-block;background:var(--primary-ink);color:#fff;font-weight:600;padding:13px 28px;border-radius:var(--radius);text-decoration:none'>" + esc(p.btnText) + "</a>" +
              "</div>" +
              (p.showImage !== false
        ...
        return "<section class='hero' style='padding:80px 20px;text-align:" + p.align + ";background:linear-gradient(120deg,var(--primary-ink),var(--accent-ink));color:#fff'><div class='wrap'>" + (p.eyebrow ? "<div style='font-size:12px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;opacity:.85;margin-bottom:14px'>" + esc(p.eyebrow) + "</div>" : "") + "<h1 style='font-size:42px'>" + esc(p.title) + "</h1><p style='font-size:18px;margin-top:16px;opacity:.92'>" + esc(p.subtitle) + "</p><p style='margin-top:26px'><a href='#main' style='background:#fff;color:var(--primary);padding:13px 26px;border-radius:var(--radius);font-weight:600;display:inline-block'>" + esc(p.btnText) + "</a></p></div></section>";
      case "postgrid":
        if (S && S.templateId === "sidebar-blog") {
        ...
        if (S && S.templateId === "review") {
          return "<section class='rv-reviews' id='main'><div class='wrap'>" +
            (p.heading ? "<h2 class='rv-h2'>" + esc(p.heading) + "</h2>" : "") +
      ...
      case "featured":
        ...
        return "<section style='padding:48px 0'><div class='wrap'><h2 style='font-size:26px;margin-bottom:20px'>" + esc(p.heading) + "</h2><b:loop values='data:posts' index='i' var='post'><b:if cond='data:i == 0'><article style='border-radius:var(--radius);overflow:hidden;background:linear-gradient(120deg,var(--primary-ink),var(--accent-ink));padding:32px;color:#fff'><h3 style='font-size:26px'><a expr:href='data:post.url' style='color:#fff'><data:post.title/></a></h3></article></b:if></b:loop></div></section>";
      case "about":
        if (S && S.templateId === "review") {
      ...
      case "columns":
        ...
        var ccells = cits.map(function (it) { return "<div style='text-align:center;padding:8px'><div style='width:54px;height:54px;border-radius:14px;background:linear-gradient(120deg,var(--primary-ink),var(--accent-ink));color:#fff;display:grid;place-items:center;font-size:24px;margin:0 auto 14px'>" + esc(it.icon || "\u2605") + "</div><h3 style='font-size:18px;margin:0 0 7px'>" + esc(it.title) + "</h3><p style='color:#828aa0;font-size:14px'>" + esc(it.text) + "</p></div>"; }).join("");
      ...
      case "cta":
        ...
        return "<section style='padding:20px'><div class='wrap'><div style='padding:48px 20px;text-align:center;border-radius:var(--radius);background:linear-gradient(120deg,var(--primary-ink),var(--accent-ink));color:#fff'><h2 style='font-size:30px'>" + esc(p.title) + "</h2><p style='margin-top:18px'><a href='" + esc(absUrl(p.btnUrl || "/")) + "' style='background:#fff;color:var(--primary);padding:13px 28px;border-radius:var(--radius);font-weight:600;display:inline-block'>" + esc(p.btnText) + "</a></p></div></div></section>";
      case "image":
      ...
      case "newsletter":
        return "<section style='padding:52px 20px;text-align:center;background:linear-gradient(120deg,var(--primary)0d,var(--accent)1a)'>"
          ...
          + "</form></div></section>";
      case "share":
        return shareCardHtml(p, "bottom") + shareScriptHtml(p);
      case "sidebar":
        var sWidgets = "";
      ...
      case "search":
        return "<section style='padding:16px 20px'><div class='wrap'>" + (p.heading ? "<h3 style='font-size:16px;margin-bottom:10px'>" + esc(p.heading) + "</h3>" : "") + "<form action='/search' method='get' role='search'><div style='display:flex;max-width:420px'><input name='q' type='search' placeholder='" + esc(p.placeholder || tpl("ค้นหาในบล็อก…","Search blog…")) + "' style='flex:1;padding:12px 16px;border:1px solid #dde;border-radius:var(--radius) 0 0 var(--radius);font-size:14px'/><button type='submit' style='padding:12px 16px;background:var(--primary-ink);color:#fff;border:0;border-radius:0 var(--radius) var(--radius) 0;cursor:pointer'>🔍</button></div></form></div></section>";
      case "footer":
        ...
        // Footer categories reuse the header's list, so there is nothing extra to fill in.
        // Chips wrap instead of stacking, which keeps 20 entries looking tidy rather than long.
        var fCats = reviewCats();
        var fHb = headerBlockOf();
        if (fHb && fHb.props.catFooter === false) fCats = [];
        var fCatMax = parseInt(fHb && fHb.props.catFooterMax != null ? fHb.props.catFooterMax : 12, 10);
        if (fCatMax > 0) fCats = fCats.slice(0, fCatMax);
        var fCatsHtml = fCats.length
        ...
        return "<footer role='contentinfo' class='site-footer'><div class='wrap'>" +
          "<div class='footer-grid" + (fCatsHtml ? " footer-grid--cats" : "") + "'>" +
      ...
      case "darkmode":
        return "<style>"
          ...
          + "b.addEventListener('click',function(){TC.toggle();});"
          + "TC.sub(function(t){b.textContent=t==='dark'?'\\u2600\\uFE0F':'\\uD83C\\uDF19';});"
          + "function place(){"
          + "var bar=document.querySelector('.site-bar');"
      ...
      case "sharebar":
        ...
        var shbBtns = (p.facebook !== false ? shbBrand("facebook", "#1877f2") : "")
          + (p.twitter !== false ? shbBrand("twitter", "#000000") : "")
          + (p.line !== false ? shbBrand("line", "#06c755") : "");
        var shbCopy = p.copy !== false
        ...
        return "<b:if cond='data:view.isSingleItem'>"
          ...
          + "<script>/*<![CDATA[*/(function(){"
          + "function cu(){var l=document.querySelector('link[rel=\"canonical\"]');var u=l?l.href:location.href;return u.replace(/([?&])m=[01](&|$)/g,function(_,a,b){return b==='&'?a:'';}).replace(/[?&]$/,'').split('#')[0];}"
          + "var t=encodeURIComponent(document.title);"
          + "[].slice.call(document.querySelectorAll('.bxb-sharebar [data-share]')).forEach(function(a){a.addEventListener('click',function(e){e.preventDefault();var u=encodeURIComponent(cu());var s=a.getAttribute('data-share'),url='';if(s==='facebook')url='https://www.facebook.com/sharer/sharer.php?u='+u;else if(s==='twitter')url='https://twitter.com/intent/tweet?text='+t+'%20'+u;else if(s==='line')url='https://social-plugins.line.me/lineit/share?url='+u;if(url){var w=window.open(url,'_blank','noopener,noreferrer');if(!w)location.href=url;}});});"
          + "var cb=document.querySelector('.bxb-shb-copy');if(cb){cb.addEventListener('click',function(){var done=function(){cb.classList.add('ok');setTimeout(function(){cb.classList.remove('ok');},1500);};if(navigator.clipboard){navigator.clipboard.writeText(cu()).then(done,done);}else{var ta=document.createElement('textarea');ta.value=cu();document.body.appendChild(ta);ta.select();try{document.execCommand('copy');}catch(e){}document.body.removeChild(ta);done();}});}"
          + "}());/*]]>*/<\/script>"
          + "</b:if>";
      case "cookie":
      ...
      case "announce":
        ...
        var anLink = anHasLink
          ? "<a class='bxb-announce-cta' href='" + esc(p.linkUrl || "#") + "' style='color:#fff;font-weight:600;text-decoration:underline;text-underline-offset:2px'>" + esc(p.linkText) + "</a>"
          : "";
      ...
      case "backtotop":
        ...
        return "<style>"
          ...
          + "var b=document.createElement('button');b.id='bxbToTop';b.type='button';b.className='bxb-totop';"
          + "b.setAttribute('aria-label','" + tpl("กลับขึ้นด้านบน", "Back to top") + "');"
          + "b.innerHTML='<svg viewBox=\"0 0 24 24\"><path d=\"M12 19V5M5 12l7-7 7 7\"/></svg>';"
          + "document.body.appendChild(b);"
          + "b.addEventListener('click',function(){window.scrollTo({top:0,behavior:'smooth'});});"
          + "var t;function upd(){if((window.pageYOffset||document.documentElement.scrollTop)>400)b.classList.add('show');else b.classList.remove('show');}"
      ...
      case "aeo":
        ...
        return "<b:if cond='data:view." + (p.onPages ? "isSingleItem" : "isPost") + "'>"
          ...
          + "var s=(el.textContent||'').replace(/\\s+/g,' ').trim();if(s.length<24)continue;"
          + "t=t?(t+' '+s):s;}"
      ...
      case "related":
        ...
        return "<b:if cond='data:view.isSingleItem'>"
          ...
          + "</b:if>";
      case "morefrom":
        var mfMode = p.mode === "random" ? "random" : "recent";
        var mfHeading = esc(p.heading || (mfMode === "random" ? tpl("สุ่มบทความ", "Random Posts") : tpl("บทความล่าสุด", "Recent Posts")));
        var mfCount = p.count || 6;
        var mfCols = p.columns || 3;
        var mfImg = p.showImage !== false;
        var mfLabel = String(p.label || "").trim();
        return "<section class='bxb-morefrom' data-mode='" + mfMode + "' data-count='" + mfCount + "' data-img='" + (mfImg ? "1" : "0") + "' data-label='" + esc(mfLabel) + "' style='padding:32px 0'>"
          + "<style>.bxb-mf-grid{display:grid;grid-template-columns:repeat(" + mfCols + ",1fr);gap:16px}@media(max-width:700px){.bxb-mf-grid{grid-template-columns:repeat(2,1fr)}}@media(max-width:440px){.bxb-mf-grid{grid-template-columns:1fr}}"
          + ".bxb-mf-card{border:1px solid var(--border,#eef);border-radius:var(--radius,12px);overflow:hidden;background:var(--bg-surface,#fff);transition:transform .15s,box-shadow .15s}"
          + ".bxb-mf-card:hover{transform:translateY(-3px);box-shadow:0 8px 22px rgba(0,0,0,.08)}"
          + ".bxb-mf-card img{width:100%;aspect-ratio:16/9;object-fit:cover;display:block;background:var(--bg-surface-2,#f1f2f9)}"
          + ".bxb-mf-card a{color:var(--text-main,#1a1b1e);text-decoration:none;font-size:14.5px;font-weight:600;line-height:1.45;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden}"
          + ".bxb-mf-date{font-size:12px;color:var(--text-subtle,#94a3b8);margin-top:6px}</style>"
          + "<div class='wrap'><h3 style='font-size:20px;font-weight:700;margin:0 0 18px'>" + mfHeading + "</h3>"
          + "<div class='bxb-mf-grid'></div></div>"
          + "</section>"
          + "<script>/*<![CDATA[*/(function(){"
          + "var sec=document.querySelector('.bxb-morefrom');if(!sec)return;"
          + "var grid=sec.querySelector('.bxb-mf-grid');"
          + "var mode=sec.getAttribute('data-mode'),count=parseInt(sec.getAttribute('data-count'),10)||6,useImg=sec.getAttribute('data-img')==='1',label=sec.getAttribute('data-label')||'';"
          + "var max=mode==='random'?50:count;"
          + "var base='/feeds/posts/default'+(label?'/-/'+encodeURIComponent(label):'');"
          + "var cur=location.href.replace(/[?#].*/,'');"
          + "fetch(base+'?alt=json&max-results='+max).then(function(r){return r.json();}).then(function(data){"
          + "var e=(data.feed&&data.feed.entry)||[];"
          + "if(mode==='random'){for(var i=e.length-1;i>0;i--){var j=Math.floor(Math.random()*(i+1));var tmp=e[i];e[i]=e[j];e[j]=tmp;}}"
          + "var posts=e.filter(function(x){var al=(x.link||[]).filter(function(l){return l.rel==='alternate';})[0];return al&&al.href.replace(/[?#].*/,'')!==cur;}).slice(0,count);"
          + "if(!posts.length){sec.parentNode&&sec.parentNode.removeChild(sec);return;}"
          + "posts.forEach(function(x){"
          + "var al=(x.link||[]).filter(function(l){return l.rel==='alternate';})[0];"
          + "var tm=x['media$thumbnail'];var img=tm?tm.url.replace(/\\/s\\d+(-c)?\\//,'/s400-c/'):null;"
          + "var t=(x.title&&x.title.$t)||'';"
          + "var dt='';try{if(x.published&&x.published.$t){dt=new Date(x.published.$t).toLocaleDateString(undefined,{year:'numeric',month:'short',day:'numeric'});}}catch(_){}"
          + "var card=document.createElement('article');card.className='bxb-mf-card';"
          + "if(useImg&&img){var im=document.createElement('img');im.src=img;im.alt='';im.loading='lazy';card.appendChild(im);}"
          + "var di=document.createElement('div');di.style.padding='12px 14px';"
          + "var h4=document.createElement('h4');h4.style.cssText='margin:0';var an=document.createElement('a');an.href=al?al.href:'#';an.textContent=t;h4.appendChild(an);di.appendChild(h4);"
          + "if(dt){var dd=document.createElement('div');dd.className='bxb-mf-date';dd.textContent=dt;di.appendChild(dd);}"
          + "card.appendChild(di);grid.appendChild(card);"
          + "});"
          + "}).catch(function(){sec.parentNode&&sec.parentNode.removeChild(sec);});"
          + "}());/*]]>*/<\/script>";
      case "progress":
      ...
      case "notfound":
        ...
        var nfsHTML = "<style>"
          ...
          + ".bxb404-code{font-size:clamp(80px,12vw,128px);font-weight:900;line-height:1;letter-spacing:-.03em;overflow-wrap:anywhere;color:" + nfsCodeClr + ";margin:0;font-family:inherit}"
  ...
  $("#dlDesign").addEventListener("click", function () { download(designMd(), "DESIGN.md", "text/markdown"); toast(tpl("ดาวน์โหลด DESIGN.md แล้ว", "DESIGN.md downloaded")); });
  ...
  /* ---------- DESIGN.md export ----------
     Writes the theme's design system in the DESIGN.md format (design.md by Google Labs),
     so the user can hand one file to any AI tool and get artwork, landing pages or ads
     that match the blog they just built. Tokens come from the live project state. */
  var DESIGN_TPL_NOTE = {
    personal:      { th: "บล็อกส่วนตัว โทนอบอุ่น อ่านสบาย ใช้ฟอนต์มีเชิงเพื่อความรู้สึกเหมือนบันทึก", en: "A personal journal: warm, unhurried, set in a serif for a written-by-hand feel." },
    travel:        { th: "บล็อกท่องเที่ยว ภาพเต็มจอเป็นพระเอก ตัวอักษรหลบให้ภาพ", en: "A travel blog where full-bleed photography leads and type stays out of its way." },
    tech:          { th: "บล็อกเทคโนโลยี มินิมอล ขอบคม เน้นความชัดเจนมากกว่าการตกแต่ง", en: "A minimal tech blog: tight radii, clarity over decoration." },
    "sidebar-blog":{ th: "บล็อกคลาสสิก 2 คอลัมน์ เนื้อหาหลักคู่กับไซด์บาร์", en: "A classic two-column blog, main column paired with a sidebar." },
    magazine:      { th: "นิตยสารข่าว หลายคอลัมน์ หนาแน่น ป้ายหมวดสีจัดเป็นตัวนำสายตา", en: "A dense multi-column magazine where saturated category tags lead the eye." },
    company:       { th: "เว็บบริษัท ดูน่าเชื่อถือ เรียบร้อย ใช้พื้นที่ว่างเยอะ", en: "A company site: trustworthy, tidy, generous with whitespace." },
    course:        { th: "เว็บคอร์สเรียน เป็นมิตร ขอบมน ชวนให้กดเริ่มเรียน", en: "A course site: friendly, rounded, built to invite a first click." },
    review:        { th: "บล็อกรีวิวสินค้าสาย Affiliate เน้นความน่าเชื่อถือและปุ่มที่กดง่าย", en: "An affiliate review blog built around credibility and an obvious next tap." }
  };
  function designMd() {
    var d = design(), tid = (S && S.templateId) || "", name = S.seo && S.seo.blogTitle || S.name || "My Blog";
    var pIn = inkFor(d.primary), aIn = inkFor(d.accent);
    var pRatio = contrastRatio(d.primary, "#ffffff"), pInRatio = contrastRatio(pIn, "#ffffff");
    var corrected = pRatio < 4.5;
    var fontStackOf = d.font === "serif" ? "Georgia, 'Times New Roman', serif"
      : d.font === "mono" ? "'Courier New', monospace"
      : "'IBM Plex Sans Thai', system-ui, sans-serif";
    var famName = d.font === "serif" ? "Georgia" : d.font === "mono" ? "Courier New" : "IBM Plex Sans Thai";
    var r = d.radius;
    var note = DESIGN_TPL_NOTE[tid] || { th: "บล็อกทั่วไป", en: "A general-purpose blog." };
    var tplName = "";
    for (var i = 0; i < TEMPLATES.length; i++) if (TEMPLATES[i].id === tid) tplName = TEMPLATES[i].name;
    function yq(v) { return '"' + String(v).replace(/"/g, '\\"') + '"'; }

    var y = [];
    y.push("---");
    y.push("version: alpha");
    y.push("name: " + yq(name));
    y.push("description: " + yq((tplName ? tplName + " theme" : "Blogger theme") + " generated with BlogKub"));
    y.push("colors:");
    y.push("  primary: " + yq(d.primary));
    y.push("  primary-ink: " + yq(pIn));
    y.push("  accent: " + yq(d.accent));
    y.push("  accent-ink: " + yq(aIn));
    y.push('  background: "#ffffff"');
    y.push('  surface: "#f7f8fc"');
    y.push('  surface-2: "#f0f1f7"');
    y.push('  border: "#e6e8f2"');
    y.push('  ink: "#1e2333"');
    y.push('  ink-2: "#4a5063"');
    y.push('  ink-3: "#6f7689"');
    y.push('  on-primary: "#ffffff"');
    y.push('  background-dark: "#0f172a"');
    y.push('  surface-dark: "#1e293b"');
    y.push('  ink-dark: "#e2e8f0"');
    y.push("typography:");
    [["h1", 36, 700, 1.2], ["h2", 26, 700, 1.25], ["h3", 20, 600, 1.3],
     ["body", 16, 400, 1.6], ["body-sm", 14, 400, 1.6], ["label", 13, 600, 1.4], ["caption", 12, 500, 1.5]
    ].forEach(function (t) {
      y.push("  " + t[0] + ":");
      y.push("    fontFamily: " + famName);
      y.push("    fontSize: " + t[1] + "px");
      y.push("    fontWeight: " + t[2]);
      y.push("    lineHeight: " + t[3]);
    });
    y.push("rounded:");
    y.push("  sm: " + Math.max(2, Math.round(r * 0.5)) + "px");
    y.push("  md: " + Math.max(4, Math.round(r * 0.75)) + "px");
    y.push("  lg: " + r + "px");
    y.push("  xl: " + Math.round(r * 1.4) + "px");
    y.push("  full: 999px");
    y.push("spacing:");
    ["xs: 4px", "sm: 8px", "md: 12px", "lg: 16px", "xl: 24px", "2xl: 40px", "3xl: 64px"].forEach(function (v) { y.push("  " + v); });
    y.push("components:");
    y.push("  button-primary:");
    y.push('    backgroundColor: "{colors.primary-ink}"');
    y.push('    textColor: "{colors.on-primary}"');
    y.push('    rounded: "{rounded.lg}"');
    y.push('    typography: "{typography.label}"');
    y.push('    padding: "{spacing.md}"');
    y.push("  card:");
    y.push('    backgroundColor: "{colors.background}"');
    y.push('    textColor: "{colors.ink}"');
    y.push('    rounded: "{rounded.xl}"');
    y.push('    padding: "{spacing.xl}"');
    y.push("  card-muted:");
    y.push('    backgroundColor: "{colors.surface}"');
    y.push('    textColor: "{colors.ink-2}"');
    y.push('    rounded: "{rounded.lg}"');
    y.push("  page:");
    y.push('    backgroundColor: "{colors.background}"');
    y.push('    textColor: "{colors.ink}"');
    y.push('    typography: "{typography.body}"');
    y.push("  page-dark:");
    y.push('    backgroundColor: "{colors.background-dark}"');
    y.push('    textColor: "{colors.ink-dark}"');
    y.push("  card-dark:");
    y.push('    backgroundColor: "{colors.surface-dark}"');
    y.push('    textColor: "{colors.ink-dark}"');
    y.push('    rounded: "{rounded.xl}"');
    y.push("  meta-text:");
    y.push('    backgroundColor: "{colors.background}"');
    y.push('    textColor: "{colors.ink-3}"');
    y.push('    typography: "{typography.caption}"');
    y.push("  divider:");
    y.push('    backgroundColor: "{colors.border}"');
    y.push("    height: 1px");
    y.push("  accent-fill:");
    y.push('    backgroundColor: "{colors.accent-ink}"');
    y.push('    textColor: "{colors.on-primary}"');
    y.push('    rounded: "{rounded.full}"');
    y.push('    typography: "{typography.label}"');
    y.push("  surface-tint:");
    y.push('    backgroundColor: "{colors.surface-2}"');
    y.push('    textColor: "{colors.ink}"');
    y.push("  brand-mark:");
    y.push('    backgroundColor: "{colors.primary}"');
    y.push('    rounded: "{rounded.md}"');
    y.push("  brand-glow:");
    y.push('    backgroundColor: "{colors.accent}"');
    y.push("    size: 320px");
    y.push("---");
    y.push("");
    ...
    b.push("");
    b.push(en
      ? "Design system for a Blogger theme built with [BlogKub](https://www.blogkub.com). Hand this file to an AI tool along with what you want made, and the result will match the blog."
      : "ระบบดีไซน์ของธีม Blogger ที่สร้างด้วย [BlogKub](https://www.blogkub.com) ส่งไฟล์นี้ให้เครื่องมือ AI พร้อมบอกว่าอยากได้อะไร ผลงานที่ได้จะเข้าชุดกับบล็อกทันที");
    b.push("");
    b.push("## Overview");
    b.push("");
    b.push(en ? note.en : note.th);
    b.push("");
    b.push(en
      ? "The layout is a single centred column with generous space between sections and one clear action per screen. Motion stays quiet: interactive elements lift slightly on hover and nothing slides in from off-screen."
      : "เลย์เอาต์เป็นคอลัมน์เดียวจัดกึ่งกลาง เว้นช่องไฟระหว่างส่วนเยอะ และมีปุ่มหลักชัดเจนหนึ่งปุ่มต่อหนึ่งหน้าจอ การเคลื่อนไหวเงียบๆ องค์ประกอบยกตัวขึ้นเล็กน้อยตอนเอาเมาส์ชี้ ไม่มีอะไรเลื่อนเข้ามาจากนอกจอ");
    b.push("");
    b.push("## Colors");
    b.push("");
    b.push(en
      ? "`primary` (" + d.primary + ") is the brand colour. It carries links, icons, borders, glows and gradient stops."
      : "`primary` (" + d.primary + ") คือสีแบรนด์ ใช้กับลิงก์ ไอคอน เส้นขอบ แสงเรือง และจุดไล่เฉดสี");
    b.push("");
    if (corrected) {
      b.push(en
        ? "`primary-ink` (" + pIn + ") is the same hue, darkened. Use it, not `primary`, wherever white text sits on a solid fill. White on `primary` measures " + pRatio.toFixed(2) + ":1, below the 4.5:1 that WCAG AA asks for at body sizes; `primary-ink` reaches " + pInRatio.toFixed(2) + ":1. The theme already applies this to its own buttons and badges."
        : "`primary-ink` (" + pIn + ") คือสีเดียวกันแต่เข้มขึ้น ใช้ตัวนี้แทน `primary` ทุกครั้งที่มีตัวหนังสือสีขาวทับบนพื้นทึบ เพราะขาวบน `primary` วัดได้ " + pRatio.toFixed(2) + ":1 ต่ำกว่ามาตรฐาน WCAG AA ที่ต้องการ 4.5:1 ส่วน `primary-ink` ได้ " + pInRatio.toFixed(2) + ":1 ตัวธีมใช้ค่านี้กับปุ่มและป้ายของตัวเองอยู่แล้ว");
    } else {
      b.push(en
        ? "White text on `primary` measures " + pRatio.toFixed(2) + ":1 and already clears WCAG AA, so `primary-ink` matches `primary`. Keep the pair in place anyway: change the brand colour later and only the ink value has to move."
        : "ตัวอักษรขาวบน `primary` วัดได้ " + pRatio.toFixed(2) + ":1 ผ่านมาตรฐาน WCAG AA อยู่แล้ว `primary-ink` จึงเท่ากับ `primary` แต่ให้เก็บคู่นี้ไว้ เพราะถ้าเปลี่ยนสีแบรนด์ทีหลัง จะขยับแค่ค่า ink ตัวเดียว");
    }
    b.push("");
    b.push(en
      ? "`accent` (" + d.accent + ") completes gradients and highlights; `accent-ink` (" + aIn + ") is its readable counterpart for text-bearing fills. The neutral ramp runs `ink` for body copy, `ink-2` for secondary copy and `ink-3` for metadata."
      : "`accent` (" + d.accent + ") ใช้ปิดท้ายการไล่เฉดสีและเน้นจุดสำคัญ ส่วน `accent-ink` (" + aIn + ") คือคู่ที่อ่านออกสำหรับพื้นที่มีตัวหนังสือ สีเทาไล่ 3 ระดับคือ `ink` สำหรับเนื้อหา `ink-2` สำหรับข้อความรอง และ `ink-3` สำหรับข้อมูลประกอบ");
    b.push("");
    b.push("## Typography");
    b.push("");
    b.push(en
      ? "One family throughout: `" + fontStackOf + "`. Headings run from 36px down to 20px, body sits at 16px and never drops below 12px. Line height stays at 1.6 for anything longer than a single line, which Thai needs more than Latin does."
      : "ใช้ชุดฟอนต์เดียวทั้งธีม `" + fontStackOf + "` หัวข้อไล่จาก 36px ลงมาถึง 20px เนื้อหาอยู่ที่ 16px และไม่เล็กกว่า 12px ระยะห่างบรรทัดคงไว้ที่ 1.6 สำหรับข้อความที่ยาวเกินหนึ่งบรรทัด ซึ่งภาษาไทยต้องการมากกว่าภาษาอังกฤษ");
    b.push("");
    b.push("## Layout");
    b.push("");
    b.push(en
      ? "Content sits in a 1080px maximum-width container with 20px of side padding. Spacing comes from the scale above and nothing in between. The single breakpoint that matters is 768px, where the header collapses into a drawer and the bottom navigation appears."
      : "เนื้อหาอยู่ในกรอบกว้างสูงสุด 1080px เว้นขอบข้าง 20px ระยะห่างทั้งหมดมาจากสเกลด้านบน ไม่มีค่าอื่นแทรก จุดตัดที่สำคัญมีจุดเดียวคือ 768px ซึ่งเมนูจะยุบเป็นลิ้นชักและแถบเมนูล่างจะโผล่ขึ้นมา");
    b.push("");
    b.push("## Shapes");
    b.push("");
    b.push(en
      ? "The base radius is " + r + "px. Small controls take about half of it, cards take about 1.4 times it, and pills are fully round. A child's radius never exceeds its parent's."
      : "ความมนพื้นฐานคือ " + r + "px ตัวควบคุมเล็กใช้ประมาณครึ่งหนึ่ง การ์ดใช้ประมาณ 1.4 เท่า และปุ่มทรงแคปซูลมนเต็ม ความมนของลูกต้องไม่มากกว่าของแม่");
    b.push("");
    b.push("## Do's and Don'ts");
    b.push("");
    b.push(en
      ? "**Do** use `primary-ink` and `accent-ink` the moment white text lands on a solid fill, and keep `primary` and `accent` for borders, icons, glows and gradient stops."
      : "**ควร** ใช้ `primary-ink` และ `accent-ink` ทันทีที่มีตัวหนังสือขาวทับบนพื้นทึบ และเก็บ `primary` กับ `accent` ไว้ใช้กับเส้นขอบ ไอคอน แสงเรือง และจุดไล่เฉดสี");
    b.push("");
    b.push(en
      ? "**Do** keep one primary action per screen. A second one of equal weight makes both weaker."
      : "**ควร** มีปุ่มหลักหน้าจอละหนึ่งปุ่ม ถ้ามีปุ่มน้ำหนักเท่ากันสองปุ่ม จะอ่อนแรงทั้งคู่");
    b.push("");
    b.push(en
      ? "**Don't** apply negative letter spacing to Thai text. It collides with the vowel and tone marks that sit above and below the baseline."
      : "**ไม่ควร** ใส่ letter-spacing ติดลบกับข้อความภาษาไทย เพราะจะชนกับสระและวรรณยุกต์ที่อยู่บนและล่างบรรทัด");
    b.push("");
    b.push(en
      ? "**Don't** invent a new spacing or radius value. Every gap should trace back to a token above."
      : "**ไม่ควร** สร้างค่าระยะห่างหรือความมนขึ้นมาใหม่ ทุกช่องไฟควรย้อนกลับไปหา token ด้านบนได้");
    b.push("");
    return y.join("\n") + b.join("\n") + "\n";
  }
  ...
  var DICT = {
    ...
    "หมวดหมู่สินค้า": "Product categories", "+ เพิ่มหมวดหมู่": "+ Add category", "ชื่อเมนูที่แสดง": "Menu label",
    "แสดงในส่วนท้ายเว็บ (Footer)": "Show in the footer", "จำนวนสูงสุดใน Footer": "Max shown in footer",
